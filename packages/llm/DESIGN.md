...
> current private `@origami/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@origami/ai`
...
- Secondary audience: Origami and other durable agent runtimes
...
import { LLM } from "@origami/ai"
import { OpenAI } from "@origami/ai/providers/openai"
...
```ts
import { Message } from "@origami/ai"
...
import { LLM, Tool } from "@origami/ai"
...
Origami and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@origami/ai/promise"
import { OpenAI } from "@origami/ai/providers/openai"
...
```ts
import { LLMSchema } from "@origami/ai/schema"
...
```ts
import { Provider, Protocol } from "@origami/ai/provider"
...
@origami/ai/providers/openai
@origami/ai/providers/anthropic
@origami/ai/providers/google
...
```
...
Origami should migrate to `generateTurn` / `streamTurn`, preserving its durable
