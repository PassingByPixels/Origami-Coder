...
interface ParserState {
  ...
  /** True once a dedicated reasoning field (reasoning_content or reasoning) has been
   *  seen this turn. When set, think-tag reasoning segments are suppressed to avoid
   *  duplicating the same thinking text that vLLM emits in BOTH the dedicated field
   *  AND leaked into `delta.content` as ` thinking... response` tags. */
...
// Route one scanned content segment into the lifecycle. A reasoning segment joins the
// same `reasoning-0` block `delta.reasoning_content` uses, so a model that mixes both
// channels (or emits several think bursts in one turn) still produces one coherent
// reasoning stream; a text segment closes reasoning first, exactly as before.
const emitSegment = (
