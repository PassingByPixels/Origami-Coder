...
export function ErrorComponent(props: { error: Error; reset: () => void; mode?: "dark" | "light" }) {
  ...
  // Safe fallback palette per mode (mirrors theme/assets/origami.json) since the
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied()
                ? "Report copied — send it to the Origami workspace maintainer."
                : "Copy the report and send it to the Origami workspace maintainer."}
            </text>
            <text fg={colors.muted}>origami {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildReport(message: string, stack: string) {
  // This fork has no public issue tracker, so the crash screen produces a plain-text
  // report the user hands to the maintainer instead of a pre-filled issue link.
  // os/terminal/version are included because they are the first things asked for.
  const head = [
    "The origami TUI crashed with an unexpected error.",
    "",
  ...
  // Keep the report pasteable: clip an over-long trace and flag the cut so a
  // truncated stack is never mistaken for a complete one.
  const MAX_STACK_LENGTH = 6000
  const body = stack.length <= MAX_STACK_LENGTH ? stack : stack.slice(0, MAX_STACK_LENGTH) + "\n... (truncated)"
  return `${head}\n\`\`\`\n${body}\n\`\`\`\n`
}
