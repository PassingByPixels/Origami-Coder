import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@origami/plugin/tui"
...
import type { useOrigamiKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof useOrigamiKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getOrigamiModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getOrigamiModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
