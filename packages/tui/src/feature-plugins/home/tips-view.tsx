import type { TuiPluginApi } from "@origami/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("origami.status"),
...
const TIPS: Tip[] = [
  ...
  "Create {highlight}origami.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/origami/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.origami/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.origami/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.origami/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.origami/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Origami from reading sensitive files",
  "Use {highlight}origami run{/highlight} for non-interactive scripting",
  "Use {highlight}origami --continue{/highlight} to resume the last session",
  "Use {highlight}origami run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}origami serve{/highlight} for headless API access to Origami",
  "Use {highlight}origami run --attach{/highlight} to connect to a running server",
  "Run {highlight}origami upgrade{/highlight} to update to the latest version",
  "Run {highlight}origami auth list{/highlight} to see all configured providers",
  "Run {highlight}origami agent create{/highlight} for guided agent creation",
  "Use {highlight}/origami{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}origami github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/origami fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.origami/themes/{/highlight} directory",
  ...
  "Run {highlight}origami debug config{/highlight} to troubleshoot configuration",
