...
export const ORIGAMI_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const ORIGAMI_MODE_KEY = "origami.mode"

export const OrigamiKeymapProvider = KeymapProvider
export const useOrigamiKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type OrigamiModeStack = ReturnType<typeof createOrigamiModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, OrigamiModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createOrigamiModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(ORIGAMI_MODE_KEY, ORIGAMI_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(ORIGAMI_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(ORIGAMI_MODE_KEY, stack.at(-1)?.mode ?? ORIGAMI_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? ORIGAMI_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(ORIGAMI_MODE_KEY, undefined)
...
export function useOrigamiModeStack() {
  return getOrigamiModeStack(useOrigamiKeymap())
}

export function getOrigamiModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("Origami mode stack is not registered for this keymap")
  return value
}
...
export function registerOrigamiKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createOrigamiModeStack(keymap)
