...
import { getOrigamiModeStack, ORIGAMI_BASE_MODE, OrigamiKeymapProvider, registerOrigamiKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerOrigamiKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when origami mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getOrigamiModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getOrigamiModeStack(keymap).push("autocomplete")
