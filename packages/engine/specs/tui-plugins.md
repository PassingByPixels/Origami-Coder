...
- Author package entrypoint is `@origami/plugin/tui`.
...
```json
{
  ...
  "plugin": ["@acme/origami-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@origami/plugin/tui`.
- `@origami/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@origami/plugin/tui"
...
- The value is a semver range checked against the running Origami version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.origami` is absent, no check is performed (backward compatible).
...
- `origami plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `origami plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.origami` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.origami`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.origami`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.origami` dir or `ORIGAMI_CONFIG_DIR`, Origami installs that directory's declared dependencies and writes:
...
Origami does NOT install `@origami/plugin` itself. That package is workspace-internal and unpublished in this fork, so the install always answered 404 — and it failed the whole reify with it, so the directory's real dependencies were not installed either. To import `@origami/plugin/tui` from a local config-scoped plugin, put the package in that directory's `node_modules` yourself.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus Origami's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Origami registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Origami pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@origami/plugin/tui`.
...
- `message` is required; `title` defaults to `"origami"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `origami.default`.
...
- Local plugins persist installed themes under the local `.origami/themes` area near the plugin config source.
...
- `--pure` / `ORIGAMI_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.origami/plugins/tui-smoke.tsx`
- Local vim plugin: `.origami/plugins/tui-vim.tsx`
- Local smoke config: `.origami/tui.json`
- Local smoke theme: `.origami/plugins/smoke-theme.json`
