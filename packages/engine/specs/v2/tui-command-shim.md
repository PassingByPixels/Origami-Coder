...
Delete `packages/engine/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/engine/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/engine/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/engine`
- TUI plugin loader tests in `packages/engine` if runtime plugin API wiring changed
