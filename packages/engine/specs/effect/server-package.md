...
Practical reference for a future `packages/server` split after the origami
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/engine`.
...
Until enough shared service code lives outside `packages/engine`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/engine`

It should not import `packages/engine` services while `packages/engine`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on origami-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/engine` implementation details.
