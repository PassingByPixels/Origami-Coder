...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from origami.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const origami = await origamiFiles(input)
  for (const file of origami) {
...
async function origamiFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "origami"),
    ...(await Filesystem.findUp(["origami.json", "origami.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "origami"))
  }
  if (Flag.ORIGAMI_CONFIG) files.push(Flag.ORIGAMI_CONFIG)
