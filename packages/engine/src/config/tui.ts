...
export class Service extends Context.Service<Service, Interface>()("@origami/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old origami.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.origami`
  // folders between cwd and home, and ORIGAMI_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.ORIGAMI_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit ORIGAMI_TUI_CONFIG override, if set.
  if (Flag.ORIGAMI_TUI_CONFIG) {
    const configFile = Flag.ORIGAMI_TUI_CONFIG
  ...
  // 4. `.origami` directories (and ORIGAMI_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".origami") || dir === Flag.ORIGAMI_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".origami") && dir !== Flag.ORIGAMI_CONFIG_DIR) continue
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    // NO `add:`, for the reason config.ts's `waitForDependencies` doc gives in
    // full: adding the unpublished `@origami/plugin` made the reify 404, so the
    // directory's OWN declared dependencies were never installed either. The
    // same defect, on the TUI's copy of the same mechanism.
    const deps = yield* Effect.forEach(data.dirs, (dir) => npm.install(dir).pipe(Effect.forkScoped), {
