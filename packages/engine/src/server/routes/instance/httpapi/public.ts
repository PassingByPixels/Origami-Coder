...
import { OrigamiHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for origami serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OrigamiHttpApi)
...
export const PublicApi = OrigamiHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "origami",
    version: "1.0.0",
