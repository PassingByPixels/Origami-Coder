  ...
  Effect.gen(function* () {
    ...
    const update = Effect.fn("SessionHttpApi.update")(function* (ctx: {
      ...
      if (ctx.payload.permission !== undefined) {
        // The auto-approve PRESET rules are replaced, everything else is merged.
        // A plain merge can only ever add, so a chat could never be taken back
        // OFF bypass through this route - and the ACP layer writes the user's
        // live preset here every time they change it, including the empty
    ...
    const abort = Effect.fn("SessionHttpApi.abort")(function* (ctx: { params: { sessionID: SessionID } }) {
      // Turn-stop: spare detached background sub-agents - they outlive the turn and
      // notify on completion. Session removal uses a different path and still cancels all.
      yield* promptSvc.cancel(ctx.params.sessionID, true)
      return true
    })
    ...
    const promptAsync = Effect.fn("SessionHttpApi.promptAsync")(function* (ctx: {
      ...
      // an accepted one is: the sender already has its own answer for why (see
      // tool/agents.ts), and reporting an error here would teach a peer that
      // its first, successful delivery had failed. Ordinary prompts carry no
      // peer rider and never reach the ledger.
      if (duplicatePeerPrompt(ctx.params.sessionID, ctx.payload.parts)) return HttpApiSchema.NoContent.make()
