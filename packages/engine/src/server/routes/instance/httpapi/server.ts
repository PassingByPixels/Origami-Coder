...
import { MoveSession } from "@origami/core/control-plane/move-session"
...
import { PermissionSaved } from "@origami/core/permission/saved"
import { ProjectV2 } from "@origami/core/project"
import { ProjectCopy } from "@origami/core/project/copy"
import { PtyTicket } from "@origami/core/pty/ticket"
...
import { CorsConfig, isAllowedCorsOrigin, type CorsOptions } from "@origami/server/cors"
import { serveUIEffect } from "@/server/shared/ui"
import { ServerAuth } from "@/server/auth"
import { InstanceHttpApi, RootHttpApi } from "./api"
import { Api } from "@origami/server/api"
...
import { handlers } from "@origami/server/handlers"
import { buildLocationServiceMap, LocationServiceMap } from "@origami/core/location-services"
import { layer as locationLayer } from "@origami/server/location"
import { sessionLocationLayer } from "@origami/server/middleware/session-location"
import { PtyEnvironment } from "@origami/server/pty-environment"
import { schemaErrorLayer as v2SchemaErrorLayer } from "@origami/server/middleware/schema-error"
...
export const webHandler = lazy(() =>
  HttpRouter.toWebHandler(routes, {
    disableLogger: true,
    // origami_change: a CHILD of the module-wide map. Engine services still
    // come from the parent - that sharing is the whole point (see the
    // `settledCommands` note in acp/service.ts) - but the ROUTE and middleware
    // layers this handler builds stay out of it. They close over the options
    // they were created with (CORS allow-list above all), and a later
    // `Server.listen` with its own options must not inherit this handler's.
