...
export const make = <A, E = never>(
  ...
  opts?: {
    ...
    /**
     * Fired when `ensureRunning` JOINS a run already in flight and therefore
     * DISCARDS the work it was handed. Without a signal here that discard is
     * completely silent: the caller gets somebody else's answer and nothing
     * anywhere records that its own prompt was dropped.
     */
  ...
  const onJoin = opts?.onJoin ?? Effect.void
    ...
    SynchronizedRef.modifyEffect(
      ...
      Effect.fnUntraced(function* (st) {
        switch (st._tag) {
          case "Running":
          case "ShellThenRun":
            // These two branches - and only these - throw `work` away. The
            // "Shell" branch below still runs it, after the shell command, so a
            // prompt there is deferred rather than lost.
            return [onJoin.pipe(Effect.andThen(() => awaitDone(st.run.done))), st] as const
          case "Shell": {
            const run = {
              id: next(),
