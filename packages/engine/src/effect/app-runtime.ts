...
export const AppLayer = AppNodeBuilderV1.build(
  LayerNode.group([
    ...
    ProviderAuth.node,
    Agent.node,
    // Already built as a ToolRegistry/SystemPrompt dependency; listed here so it
    // is a DECLARED service of the runtime, which is what lets the Flock ext
    // methods yield it (same reason Skill.node is listed for `list_skills`).
    FlockRouting.node,
    Skill.node,
    Discovery.node,
    AgentPlugins.node,
    ...
    SessionSummary.node,
    SessionPrompt.node,
    // Both listed, not just the runner: the Collab ext methods yield the store
    // directly for the reads and the cap write, so it has to be a DECLARED
    // service of the runtime and not only a transitive dependency.
    CollabStore.node,
    CollabRunner.node,
    Instruction.node,
    LLM.node,
