 ...
 * engine's OWN flat markers, written by `SessionProcessor.notice` - which types
 * them `Record<string, string>` - plus the compaction/unknown-finish writers:
 * `origami_retry`, `origami_degraded`, `origami_truncated`,
 * `origami_unknown_finish`, `origami_compaction_stop`.
 *
 ...
 * messages array - so one stored retry notice makes every later turn in that
 * session fail with AI_InvalidPromptError, permanently. Keep only the entries
 * that are actually provider-shaped.
 */
...
export function fromError(
  ...
  switch (true) {
    ...
    // A stream that died after the request was accepted. The AI SDK retries
    // only `doStream`, so anything that goes wrong once the body is flowing
    // arrives here raw — no status, no headers, no APICallError. Typed as a
    // retryable APIError so the ordinary retry ladder can carry it, and marked
    // so `SessionRetry` can spend the family's own tighter budget on it.
