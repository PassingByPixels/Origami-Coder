...
/**
 * This provider registers dynamically (DCR). The 2026-07-28 revision formally
 * DEPRECATES DCR in favour of Client ID Metadata Documents (CIMD, SEP-991),
 * with a support window of at least twelve months — so DCR is the correct path
 * today and the one this file keeps.
 *
 * CIMD is not a code change here: it needs a `client.json` served from a stable
 * HTTPS origin we control, which is infrastructure, not client work. It is
 * therefore scoped as its own follow-up ticket rather than half-built here.
 */
...
export class McpOAuthProvider implements OAuthClientProvider {
  ...
  get clientMetadata(): OAuthClientMetadata {
    return {
      ...
      client_name: "Origami",
      client_uri: "https://origami.gratis",
