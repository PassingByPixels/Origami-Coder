...
import { McpEvent } from "@origami/schema/mcp-event"
...
const CLIENT_OPTIONS = {
  capabilities: {
    ...
    // sampling: {},
    // Elicitation is how a 2026-07-28 server asks the human a question, and on
    // that era it arrives as an `input_required` result the SDK fulfils through
    // the handler registered in `createClient`. The capability must be declared
    // or the SDK refuses the handler registration.
    elicitation: {},
    // Deprecated by SEP-2577 on 2026-07-28 but still the only way a 2025-era
    // server learns the workspace root, so it stays for the legacy arm.
    // https://github.com/anomalyco/opencode/issues/2308
    roots: {},
    // https://github.com/anomalyco/opencode/issues/28567
    // tasks: {},
  },
  // `inputRequired.autoFulfill` is left at its default `true`: the SDK runs the
  // input_required retry loop itself through the registered handlers, so
  // `callTool` keeps returning a plain result and neither call site needs a
  // bespoke retry.
  //
  // The SDK now walks list pagination for us (see mcp/catalog.ts). Its own
  // default cap is 64; keep the limit this codebase has always enforced instead
  // of silently tightening it on every user with a large server.
  listMaxPages: McpCatalog.MAX_LIST_PAGES,
  // `defaultCacheTtlMs` stays at its default 0: a list result that arrives
  // WITHOUT a server `ttlMs` is stored (the derived tools index needs it) but
  // never served from cache. That is exactly today's behaviour — every list
  // refetches — so 2026-07-28 servers gain caching and nothing else changes.
...
/**
 * Probe `server/discover` at connect and fall back to the plain 2025
 * `initialize` handshake when the server is not 2026-07-28. This is the SDK's
 * own probe: it gets the transport-aware timeout verdict right (stdio silence
 * means legacy, HTTP silence means outage) and, on the SDK's stdio transport,
 * runs the probe on a disposable sibling process so a legacy server that dies
 * on an unknown pre-`initialize` request is still reached on a fresh pipe.
 * Hand-rolling the probe would get all three of those wrong.
 *
 * The probe timeout MUST be bound to the server's own connect budget. Left at
 * the SDK default the probe inherits the standard request timeout, which is far
 * longer than a user-configured `timeout`: our outer `withTimeout` would then
 * abandon the connect while the probe was still waiting, and the legacy
 * fallback — the entire reason a 2025 server still works — would never run. Half
 * the budget goes to reaching a verdict, half to the handshake that follows it.
 */
const negotiation = (timeout: number) => ({
  mode: "auto" as const,
  probe: { timeoutMs: Math.max(1, Math.floor(timeout / 2)) },
})

function createClient(directory: string, bridge: EffectBridge.Shape, timeout: number) {
  const client = new Client(
    { name: "origami", version: InstallationVersion },
    { ...CLIENT_OPTIONS, versionNegotiation: negotiation(timeout) },
  )
  client.setRequestHandler("roots/list", () => Promise.resolve({ roots: [{ uri: pathToFileURL(directory).href }] }))
  // Serves both eras: a 2025 server sends `elicitation/create` as a request, a
  // 2026-07-28 server returns `input_required` and the SDK dispatches it here.
  client.setRequestHandler("elicitation/create", (request) => bridge.promise(McpElicitation.handle(request)))
  return client
}

const ProtocolEra = Schema.Literals(["modern", "legacy"])

const StatusConnected = Schema.Struct({
  status: Schema.Literal("connected"),
  /**
   * Which protocol era the connection negotiated: `modern` for a 2026-07-28+
   * `server/discover`, `legacy` for the 2025 `initialize` handshake. Optional
   * because a client can be connected before the era is readable.
   */
  era: Schema.optional(ProtocolEra),
}).annotate({
...
/**
 * The connected status for `client`, carrying the era it negotiated. Read from
 * the SDK rather than inferred from the config, because `mode: 'auto'` can put
 * a server we configured as remote on either era depending on what it answered.
 */
export function connectedStatus(client: MCPClient): Status {
  const era = client.getProtocolEra()
  return era ? { status: "connected", era } : { status: "connected" }
}
...
export interface McpTool {
  ...
  /**
   * The agent-plugins.org plugin that supplied this tool's server, when one did.
   * Carried here because the permission gate in `session/tools.ts` sees only the
   * flattened tool key by then, and `plugin:<name>:*` rules need the plugin.
   */
...
export class Service extends Context.Service<Service, Interface>()("@origami/MCP") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const connectTransport = Effect.fn("MCP.connectTransport")(function* (transport: Transport, timeout: number) {
      ...
      // The bridge MUST be made here, not once at layer build: it captures the
      // InstanceRef of the fiber that makes it, and the layer is built outside
      // any instance. A layer-level bridge would run the elicitation handler's
      // `Question.ask` against a different instance's pending map, so the
      // question would be raised where nobody is listening.
      ...
      return yield* Effect.acquireUseRelease(
        ...
        (t) =>
          Effect.tryPromise({
            try: () => {
              const client = createClient(directory, bridge, timeout)
    ...
    const connectRemote = Effect.fn("MCP.connectRemote")(function* (
      ...
      for (const { name, transport } of transports) {
            ...
            if (isAuthError) {
              if (lastError.message.includes("registration") || lastError.message.includes("client_id")) {
                ...
                return events
                  .publish(TuiEvent.ToastShow, {
                    title: "MCP Authentication Required",
                    message: `Server "${key}" requires authentication. Run: origami mcp auth ${key}`,
                    variant: "warning",
                    duration: 8000,
                  })
                  .pipe(Effect.ignore, Effect.as(undefined))
        ...
        if (result) return { client: result.client, status: connectedStatus(result.client) }
    ...
    function watch(s: State, name: string, client: MCPClient, bridge: EffectBridge.Shape, timeout?: number) {
      ...
      client.setNotificationHandler("notifications/tools/list_changed", async () => {
        ...
        // `refresh`, not the default `use`: the notification says the held entry
        // is stale. The SDK's own eviction runs first but is fire-and-forget,
        // so this refetch must not depend on it having landed (see
        // McpCatalog.ListCacheMode).
        const listed = await bridge.promise(McpCatalog.defs(client, timeout, "refresh"))
        if (!listed) return
        if (s.clients[name] !== client || s.status[name]?.status !== "connected") return
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("MCP.state")(function* () {
        ...
        const bridge = yield* EffectBridge.make()
        // Plugin servers first so a user's own `mcp` entry of the same name
        // overrides one a plugin brought in, never the other way round. They are
        // also seeded into `s.config`, which is the runtime-known server map that
        // `status`, `connect`/`disconnect` and `requestTimeout` all read - without
        // that a plugin server would connect but never appear in `mcp list`.
        const pluginServers = yield* agentPlugins.mcpServers()
        const config = { ...pluginServers, ...(cfg.mcp ?? {}) }
        const s: State = {
          config: { ...pluginServers },
          status: {},
          clients: {},
          defs: {},
          instructions: {},
    ...
    const tools = Effect.fn("MCP.tools")(function* () {
      ...
      for (const [clientName, client] of Object.entries(s.clients)) {
        ...
        const plugin = yield* agentPlugins.owner(clientName)
        for (const def of listed) {
          result[McpCatalog.toolName(clientName, def.name)] = { def, client, timeout, ...(plugin ? { plugin } : {}) }
        }
      }
      return result
    })
    ...
    const startAuth = Effect.fn("MCP.startAuth")(function* (mcpName: string) {
      ...
      const directory = yield* InstanceState.directory
      // Same instance-capture rule as connectTransport: made here, inside the
      // instance, so a later elicitation reaches the right question service.
      const bridge = yield* EffectBridge.make()

      return yield* Effect.tryPromise({
        try: () => {
          const client = createClient(directory, bridge, mcpConfig.timeout ?? DEFAULT_TIMEOUT)
...
export const node = LayerNode.make({
  ...
  deps: [
    ...
    Config.node,
    McpBrowser.node,
    // The elicitation handler asks through the question tool's UX.
    Question.node,
    AgentPlugins.node,
  ],
})
