...
import type * as ModelsDev from "@origami/core/models-dev"
...
function aliasesCollidingToolName(model: Provider.Model): boolean {
  return OPENAI_TOOL_SEARCH_COLLISION_NPM.has(model.api.npm)
}
    ...
    if (msg.role === "assistant" && Array.isArray(msg.content)) {
      return {
        ...
        content: msg.content.map((part) => {
          ...
          if (part.toolName !== TOOL_SEARCH_WIRE_NAME) return part
          return { ...part, toolName: TOOL_SEARCH_WIRE_ALIAS }
        }),
      }
    }
...
export function renameCollidingTool<T>(tools: Record<string, T>, model: Provider.Model): Record<string, T> {
  ...
  if (!(TOOL_SEARCH_WIRE_NAME in tools)) return tools
  const { [TOOL_SEARCH_WIRE_NAME]: toolSearch, ...rest } = tools
  return { ...rest, [TOOL_SEARCH_WIRE_ALIAS]: toolSearch }
}
...
/**
 * The gpt-5 ids this engine sets a reasoning effort for: `-chat` accepts none
 * at all and `-pro` fixes its own. Mirrors the pair of nested tests the hosted
 * branch in `options()` uses, so the gateway branch and the hosted branch
 * cannot drift on which ids are in scope.
 */
function gpt5AcceptsReasoningEffort(apiId: string) {
  return apiId.includes("gpt-5") && !apiId.includes("gpt-5-chat") && !apiId.includes("gpt-5-pro")
}
...
export function options(input: {
  ...
  if (
    ...
    (input.model.providerID === "origami" && ["kimi-k2-thinking", "glm-4.6"].includes(input.model.api.id))
  ) {
  ...
  if (input.model.api.id.includes("gpt-5") && !input.model.api.id.includes("gpt-5-chat")) {
    if (!input.model.api.id.includes("gpt-5-pro")) {
      result["reasoningEffort"] = GPT5_REASONING_EFFORT_DEFAULT
      if (
        input.model.api.npm === "@ai-sdk/openai" ||
        input.model.api.npm === "@ai-sdk/azure" ||
        input.model.api.npm === "@ai-sdk/github-copilot" ||
