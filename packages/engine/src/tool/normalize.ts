 ...
 * invents content.
 *
 * Syntactic repair of malformed or truncated JSON is deliberately not done, for
 * any tool: a "repaired" shell command that happens to satisfy the schema is a
 * wrong command handed to a shell, so unparseable input is reported back.
 *
 * The caller is Tool.wrap, the only validation seam either runtime has: the AI SDK builds
 * tools with `jsonSchema(plainObject)`, whose `validate` is undefined, so it rejects
 * unparseable JSON and passes every shape error through untouched.
 */
...
export type Options = {
   ...
   * aliasing can never introduce a key the tool does not accept. Absent means
   * alias nothing: a rename decided without the tool's parameters is a guess.
   */
...
/**
 * Comparison form of a key name: separators dropped, case folded, so `x_labels`,
 * `x-labels`, `X Labels` and `xlabels` all reduce to `xlabels`. Matching on the
 * underscore alone would leave kebab and run-together spellings unaliased.
 */
...
function unalias(input: Record<string, unknown>, keys: readonly string[]) {
  ...
  // Two sent keys reducing to one declared name would overwrite each other on
  // an accident of key order, so that form claims neither.
 ...
 *  from the tool being called is what keeps aliasing correct per tool. */
export function parameterKeys(schema: unknown): readonly string[] {
  return Object.keys(properties(schema) ?? {})
}

/** Top-level keys of a payload the tool does not declare. Empty when there is
 *  nothing to judge — refusing against an unknown key list refuses every call. */
 ...
 *  whose canonical form contains, or is contained by, the one sent. Two matches,
 *  or none, suggests nothing rather than guessing. */
...
/**
 * The corrective message handed back when a call cannot be rescued: the tool,
 * the flat key names it expects, and which are missing or unrecognised. ""
 * when the schema carries no properties.
 *
 * `received` must be the payload as the DECODER saw it, never as it arrived, or
 * keys normalisation already fixed get reported as missing.
 */
