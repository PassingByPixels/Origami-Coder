...
import { CodeMode, Tool as SandboxTool, toolError } from "@origami/codemode"
import { MCP } from "@/mcp"
...
const invokeChildTool = Effect.fn("CodeMode.invokeChildTool")(function* (input: {
  ...
  const result: CallToolResult = yield* Effect.gen(function* () {
    ...
    return yield* Effect.promise(async () => {
      // Same reason as session/tools.ts: an MCP elicitation is fulfilled inside
      // callTool, so the asking session has to be established around it.
      const raw = await McpElicitation.withCaller(
        { sessionID: input.ctx.sessionID, messageID: input.ctx.messageID, callID: input.callID },
        () =>
