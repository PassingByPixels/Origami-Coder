...
import { IdeEvent } from "@origami/schema/ide-event"
...
export function alreadyInstalled() {
  return process.env["ORIGAMI_CALLER"] === "vscode" || process.env["ORIGAMI_CALLER"] === "vscode-insiders"
}
...
export async function install(ide: (typeof SUPPORTED_IDES)[number]["name"]) {
  ...
  const p = await Process.run([cmd, "--install-extension", "sst-dev.origami"], {
    nothrow: true,
  })
