...
export function withoutSampling(body: BodyInit | null | undefined): BodyInit | null | undefined {
  ...
  try {
    ...
    if (!("temperature" in record) && !("top_p" in record)) return body
    delete record["temperature"]
    delete record["top_p"]
    return JSON.stringify(record)
  } catch {
    return body // not JSON (or not ours to touch) — pass it through untouched
  }
}
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            const url = rewritten ? new URL(codexApiEndpoint) : parsed

            const requestInit = {
              ...init,
              // Sampling is stripped ONLY where the URL was actually rewritten to
              // the ChatGPT backend — see withoutSampling for why not chat.params.
              body: rewritten ? withoutSampling(init?.body) : init?.body,
              headers,
            }
