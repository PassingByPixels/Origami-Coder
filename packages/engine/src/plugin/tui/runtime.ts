...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@origami/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@origami/core/effect/app-node-builder"
import { errorData, errorMessage } from "@origami/tui/util/error"
...
import { hasTheme, upsertTheme } from "@origami/tui/context/theme"
import { Global } from "@origami/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@origami/core/util/flock"
import { Flag } from "@origami/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@origami/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@origami/core/v1/config/plugin"
import { createCommandShim } from "@origami/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@origami/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".origami"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".origami", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".origami", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.ORIGAMI_PURE ? [] : pluginOrigins
    if (Flag.ORIGAMI_PURE && pluginOrigins.length) {
    }
