...
export const DEPRECATED_PLUGIN_PACKAGES = ["origami-openai-codex-auth", "origami-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, origamiVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(origamiVersion) || semver.major(origamiVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.origami
  if (typeof range !== "string") return
  if (!semver.satisfies(origamiVersion, range)) {
    throw new Error(`Plugin requires origami ${range} but running ${origamiVersion}`)
  }
}
