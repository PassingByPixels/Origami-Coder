// Shared type vocabulary for the direct interactive mode (`origami --mini`).
//
...
import type { OrigamiClient, PermissionRequest, QuestionRequest, ToolPart } from "@origami/sdk/v2"
import type { TuiConfig } from "@origami/tui/config"
...
type PromptModel = Parameters<OrigamiClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<OrigamiClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<OrigamiClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<OrigamiClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<OrigamiClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<OrigamiClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<OrigamiClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<OrigamiClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<OrigamiClient["question"]["reject"]>[0]
