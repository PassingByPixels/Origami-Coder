...
import type { PermissionRequest } from "@origami/sdk/v2"
...
export function permissionAlwaysLines(request: PermissionRequest): string[] {
  // An EMPTY always list is a tool saying "there is no answer that covers the
  // next call" - `screenshot` is the first, because a whole-screen grab can
  // hold anything on the desktop. Permission.reply loops over this array to
  // build the standing rules, so an empty one grants nothing and the next call
  // asks again. Without this branch the prose promised a standing approval and
  // then listed no patterns, which reads as a bug in the picker.
  if (request.always.length === 0) {
    return [`This will not carry over: ${request.permission} asks again every time.`]
  }

  if (request.always.length === 1 && request.always[0] === "*") {
    return [`This will allow ${request.permission} until Origami is restarted.`]
  }

  return [
    "This will allow the following patterns until Origami is restarted.",
