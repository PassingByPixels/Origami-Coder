...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: origami mcp add")
      return
    }
    ...
    for (const [name, serverConfig] of servers) {
      ...
      if (!status) {
        ...
        // Which protocol era the connection negotiated. Worth showing: a server
        // on `legacy` is running on the 2025 revision's deprecation clock.
        if (status.era) statusText += ` (${status.era})`
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in origami.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
  ...
  handler: Effect.fn("Cli.mcp.add")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      if (args.name) {
        ...
        const configPath = await McpConfigWrite.resolveConfigPath(Global.Path.config, true)
        await McpConfigWrite.addServer(configPath, args.name, mcpConfig)
      ...
      const [projectConfigPath, globalConfigPath] = await Promise.all([
        McpConfigWrite.resolveConfigPath(ctx.worktree),
        McpConfigWrite.resolveConfigPath(Global.Path.config, true),
      ])
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., origami x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        await McpConfigWrite.addServer(configPath, name, mcpConfig)
...
/**
 * `mode: 'auto'` so the debug client runs the SAME negotiation the engine runs:
 * probe `server/discover`, fall back to the 2025 `initialize` handshake. Debug
 * output that used a different posture from the real client would be worse than
 * no output.
 */
const NEGOTIATING_DEBUG_OPTIONS = { versionNegotiation: { mode: "auto" } } as const

function reportEra(client: Client) {
  const era = client.getProtocolEra()
  const version = client.getNegotiatedProtocolVersion()
  if (!era) {
    prompts.log.warn("Protocol era: unknown (connection reported none)")
    return
  }
  prompts.log.info(`Protocol era: ${era}${version ? ` (${version})` : ""}`)
  if (era === "legacy") {
    prompts.log.info("  This server speaks the 2025 revision; it is inside the 12-month deprecation window.")
  }
}

/**
 * The modern branch of the debug probe: connect with the SDK's own
 * `server/discover` negotiation and report which era it landed on. Deliberately
 * NOT a hand-built `server/discover` POST — the SDK owns the probe's timeout
 * verdict, retry policy and fallback, and a re-implementation here would report
 * on a probe the engine never runs.
 */
async function probeEra(url: string, headers?: Record<string, string>) {
  const client = new Client({ name: "origami-debug", version: InstallationVersion }, NEGOTIATING_DEBUG_OPTIONS)
  ...
  try {
    ...
    prompts.log.warn(`Protocol negotiation failed: ${error instanceof Error ? error.message : String(error)}`)
  } finally {
...
export const McpDebugCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.debug")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      //
      // This raw POST is deliberately a LEGACY probe and is labelled as one:
      // `LATEST_PROTOCOL_VERSION` is "2025-11-25", the newest revision that
      // still HAS an `initialize` handshake. The 2026-07-28 revision removed it,
      // so a modern-only endpoint will refuse this body — which is information,
      // not a failure. The modern side is probed further down by the SDK's own
      // `server/discover` negotiation, because hand-rolling that probe gets the
      // transport-aware timeout and fallback semantics wrong.
      try {
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "origami-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })

        spinner.stop(`Legacy (${LATEST_PROTOCOL_VERSION}) initialize probe: ${response.status} ${response.statusText}`)
        ...
        if (response.status === 401) {
          ...
          try {
            ...
            if (error instanceof UnauthorizedError) {
              ...
              // Check if dynamic registration would be attempted. DCR is
              // formally deprecated as of 2026-07-28 in favour of Client ID
              // Metadata Documents (CIMD, SEP-991), but still works and is
              // still the path here — CIMD needs a client.json hosted on our own
              // infrastructure and is scoped as its own ticket.
              const clientInfo = await authProvider.clientInformation()
              if (clientInfo) {
                prompts.log.info(`Client ID available: ${clientInfo.client_id}`)
              } else {
                prompts.log.info("No client ID - dynamic registration will be attempted (DCR, deprecated but active)")
              }
            } else {
          ...
          await probeEra(serverConfig.url, serverConfig.headers)
        } else {
          ...
          // A modern-only endpoint refuses the legacy `initialize` body, so a
          // non-2xx here is not proof the server is unreachable.
          await probeEra(serverConfig.url, serverConfig.headers)
        }
      } catch (error) {
