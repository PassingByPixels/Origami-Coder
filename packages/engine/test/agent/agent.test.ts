...
it.instance("plan agent denies edits except .origami/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".origami/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)

it.instance("plan agent whitelists GLOBAL plans under ~/.origami, not the pre-move data dir", () =>
  Effect.gen(function* () {
    const plan = yield* load((svc) => svc.get("plan"))
    expect(plan).toBeDefined()
    // Non-vcs (global) plans now live under ~/.origami/plans - Session.plan() writes
    // there and the plan agent must be permitted to write outside the worktree there.
    const globalPlan = path.join(Global.Path.origami, "plans", "1783000000000-foo.md")
    expect(Permission.evaluate("external_directory", globalPlan, plan!.permission).action).toBe("allow")
    // The pre-move ~/.local/share/origami/plans location is no longer whitelisted;
    // this fails loudly if any of the 4 permission sites drift back to Global.Path.data.
    const oldPlan = path.join(Global.Path.data, "plans", "1783000000000-foo.md")
    expect(Permission.evaluate("external_directory", oldPlan, plan!.permission).action).not.toBe("allow")
  }),
)
...
it.instance("plan agent denies the general subagent by default", () =>
  Effect.gen(function* () {
    const plan = yield* load((svc) => svc.get("plan"))
    expect(plan).toBeDefined()
    // Plan mode delegates only to the read-only 'explore' sub-agent; general,
    // build, and custom (execute-capable) types are denied so it can't fan out
    // executing sub-agents mid-plan (fork intent, commit 7914e735).
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".origami", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.ORIGAMI_TEST_HOME
      process.env.ORIGAMI_TEST_HOME = test.directory
...
it.instance(
  "defaultAgent falls through to deep-plan when build and plan are both disabled",
  () =>
    Effect.gen(function* () {
      // The fall-through order is the natives table's own order, so a new
      // primary agent lands in this chain whether or not anyone thought about
      // it. Pinned rather than left implicit: it is the only observable
      // statement of where deep-plan sits, and the test below depends on it.
      const agent = yield* load((svc) => svc.defaultAgent())
      expect(agent).toBe("deep-plan")
    }),
  {
    config: {
      agent: {
  ...
  () => expectDefaultAgentError("no primary visible agent found"),
  {
    config: {
      agent: {
        build: { disable: true },
        plan: { disable: true },
        // ALL of them means all of them. Deep plan is the third visible
        // primary; without this line the fixture stopped saying what its name
        // says and the throw was never reached.
        "deep-plan": { disable: true },
      },
    },
  },
)
