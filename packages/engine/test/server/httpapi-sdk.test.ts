...
type Sdk = ReturnType<typeof createOrigamiClient>
...
function serverFetch(
  ...
  return HttpServer.HttpServer.use((server) =>
    Effect.sync(() => {
      void serverPath
      Flag.ORIGAMI_SERVER_PASSWORD = input?.password
      Flag.ORIGAMI_SERVER_USERNAME = input?.username
...
function writeProjectSkill(dir: string) {
  return FSUtil.Service.use((fs) =>
    fs.writeWithDirs(
      path.join(dir, ".origami", "skills", "project-rest-skill", "SKILL.md"),
      `---
...
describe("HttpApi SDK", () => {
  ...
  httpapi(
    ...
    withProject("raw", { setup: writeStandardFiles }, ({ directory }) =>
      Effect.gen(function* () {
        ...
        expect(request!.headers.has("x-origami-directory")).toBe(false)
        expect(request!.headers.has("x-origami-workspace")).toBe(false)
      }),
    ),
  )
    ...
    ({ directory }) =>
      Effect.gen(function* () {
        ...
        const badSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("origami", "wrong") },
        })
        const bad = yield* capture(() => badSdk.file.read({ path: "hello.txt" }))
        const goodSdk = yield* client("raw", directory, {
          password: "secret",
          headers: { authorization: authorization("origami", "secret") },
        })
  ...
  // The channel the ACP layer writes the per-chat auto-approve preset on. A plain
  // merge can only ADD, so a chat could never be taken back off bypass through
  // this route; the preset rules are swapped and everything else is kept.
  serverPathParity("swaps the auto-approve preset on a session row instead of stacking it", (serverPath) =>
    withStandardProject(serverPath, ({ sdk }) =>
      Effect.gen(function* () {
        const configured = { permission: "bash", pattern: "git status", action: "allow" } as const
        const created = yield* capture(() => sdk.session.create({ title: "presets", permission: [configured] }))
        const sessionID = String(record(created.data).id)
        const bypass = yield* capture(() =>
          sdk.session.update({ sessionID, permission: [{ permission: "*", pattern: "*", action: "allow" }] }),
        )
        // The empty ruleset is what "back to default" sends. It has to clear the
        // grant without touching the rule the chat was configured with.
        const cleared = yield* capture(() => sdk.session.update({ sessionID, permission: [] }))

        expect(record(bypass.data).permission).toEqual([configured, { permission: "*", pattern: "*", action: "allow" }])
        expect(record(cleared.data).permission).toEqual([configured])

        return { statuses: statuses({ created, bypass, cleared }) }
      }),
    ),
  )
