...
import { SessionInputTable, SessionMessageTable, SessionTable } from "@origami/core/session/sql"
...
const originalWorkspaces = Flag.ORIGAMI_EXPERIMENTAL_WORKSPACES
...
afterEach(async () => {
  Flag.ORIGAMI_EXPERIMENTAL_WORKSPACES = originalWorkspaces
...
describe("session HttpApi", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-origami-directory": test.directory }
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* request(`/api/session/${session.id}/message`, {
          headers: { "x-origami-directory": test.directory },
        })
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        Flag.ORIGAMI_EXPERIMENTAL_WORKSPACES = true
        ...
        const created = yield* requestJson<Session.Info>(`${SessionPaths.create}?workspace=${workspace.id}`, {
          method: "POST",
          headers: { "x-origami-directory": test.directory, "content-type": "application/json" },
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const currentDir = path.join(test.directory, "packages", "engine", "src")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-origami-directory": hint, "content-type": "application/json" }
        const created = yield* requestJson<Session.Info>(SessionPaths.create, {
          method: "POST",
          headers,
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = { "x-origami-directory": globalWorktreeSentinel, "content-type": "application/json" }
