...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ origami }) =>
    Effect.gen(function* () {
      const result = yield* origami.spawn(["--replay-limit", "10"])

      origami.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ origami }) =>
    Effect.gen(function* () {
      const result = yield* origami.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ origami }) =>
    Effect.gen(function* () {
      const result = yield* origami.spawn(["--mini", "--port", "4096"])
