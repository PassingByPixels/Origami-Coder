...
describe("origami acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ origami }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ origami }))
        ...
        expect(initialized.agentInfo?.name).toBe("Origami")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ origami }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ origami })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("origami-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "origami-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.ORIGAMI_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ origami }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("origami-login")
