...
describe("origami read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // ORIGAMI_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ origami }) =>
      Effect.gen(function* () {
        const r = yield* origami.spawn(["mcp", "list"])
        origami.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ origami }) =>
      Effect.gen(function* () {
        const r = yield* origami.spawn(["providers", "list"])
        origami.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ origami }) =>
      Effect.gen(function* () {
        const r = yield* origami.spawn(["models"])
        origami.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ origami }) =>
      Effect.gen(function* () {
        const r = yield* origami.spawn(["agent", "list"])
        origami.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh ORIGAMI_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ origami }) =>
      Effect.gen(function* () {
        const r = yield* origami.spawn(["session", "list"])
        origami.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ origami }) =>
      Effect.gen(function* () {
        const r = yield* origami.spawn(["stats"])
        origami.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under ORIGAMI_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ origami }) =>
      Effect.gen(function* () {
        const r = yield* origami.spawn(["db", "path"])
        origami.expectExit(r, 0, "db path")
