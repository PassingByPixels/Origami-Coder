...
import { OrigamiClient, type GlobalEvent } from "@origami/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<OrigamiClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<OrigamiClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<OrigamiClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<OrigamiClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<OrigamiClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://origami.test"),
...
function sdk(
  input: {
    ...
    subscribe?: OrigamiClient["event"]["subscribe"]
    globalEvent?: OrigamiClient["global"]["event"]
    promptAsync?: OrigamiClient["session"]["promptAsync"]
    status?: OrigamiClient["session"]["status"]
    messages?: OrigamiClient["session"]["messages"]
    children?: OrigamiClient["session"]["children"]
    permissions?: OrigamiClient["permission"]["list"]
    questions?: OrigamiClient["question"]["list"]
  } = {},
) {
  const client = new OrigamiClient()

  const subscribe: OrigamiClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: OrigamiClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: OrigamiClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: OrigamiClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: OrigamiClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: OrigamiClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: OrigamiClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: OrigamiClient["question"]["list"] = input.questions ?? (() => ok([]))
