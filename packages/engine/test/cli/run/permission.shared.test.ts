...
describe("run permission shared", () => {
  ...
  test("formats always-allow copy for wildcard and explicit patterns", () => {
    expect(permissionAlwaysLines(req({ permission: "bash", always: ["*"] }))).toEqual([
      "This will allow bash until Origami is restarted.",
    ])
  ...
  test("an empty always list says the answer will NOT carry over", () => {
    // `screenshot` asks every time on purpose: it passes no always-patterns, so
    // Permission.reply has nothing to turn into a standing rule. The copy has to
    // say that rather than promise an allow that never happens.
    expect(permissionAlwaysLines(req({ permission: "screenshot", always: [] }))).toEqual([
      "This will not carry over: screenshot asks again every time.",
    ])
  })
})
