...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/origami.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/origami.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/origami.jsonc", dir: ".origami", suggestion: "origami" },
        expected:
          'Directory ".origami" in /tmp/origami.jsonc is not valid. Rename the directory to "origami" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/origami.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/origami.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Try: `origami models` to list available models",
      "Or check your config (origami.json) provider/model names",
    ].join("\n")
