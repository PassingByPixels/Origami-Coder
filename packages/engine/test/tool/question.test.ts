...
describe("tool.question", () => {
  ...
  it.instance("answers a BATCH of questions in one call, each mapped to its own question", () =>
    Effect.gen(function* () {
      const question = yield* Question.Service
      const toolInfo = yield* QuestionTool
      const tool = yield* toolInfo.init()
      const questions = [
        { question: "Which parser?", header: "Parser", options: [{ label: "PEG", description: "" }] },
      ...
      const fiber = yield* tool.execute({ questions }, ctx).pipe(Effect.forkScoped)
      const item = yield* pending(question)
      // ONE request holds all three — the tool asked once, not three times.
      expect(item.questions).toHaveLength(3)
      yield* question.reply({ requestID: item.id, answers: [["PEG"], ["SQLite"], ["Dark"]] })

      const result = yield* Fiber.join(fiber)
      expect(result.title).toBe("Asked 3 questions")
      expect(result.output).toContain(`"Which parser?"="PEG"`)
      expect(result.output).toContain(`"Which store?"="SQLite"`)
      expect(result.output).toContain(`"Which theme?"="Dark"`)
      expect(result.metadata.answers).toEqual([["PEG"], ["SQLite"], ["Dark"]])
    }),
  )

  it.instance("CANCEL returns a normal tool result so the turn continues, not a failure", () =>
    Effect.gen(function* () {
      const question = yield* Question.Service
      const toolInfo = yield* QuestionTool
      const tool = yield* toolInfo.init()
      ...
      // Joining SUCCEEDS. Left uncaught, the RejectedError reaches
      // SessionProcessor.failToolCall, which sets ctx.blocked and halts the run
      // — cancelling a clarifying question would kill the turn.
      const result = yield* Fiber.join(fiber)
      expect(result.title).toBe("Cancelled 2 questions")
      // The model is told plainly that it was cancelled, and told not to re-ask.
      expect(result.output).toContain("CANCELLED")
      expect(result.output).toContain("Do not ask them again")
