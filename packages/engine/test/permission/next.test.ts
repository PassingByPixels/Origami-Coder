...
// Sub-agent stall — the engine half. Concurrent children of one session ask for the
// SAME permission on the SAME pattern; each used to park its own pending request, so
// every one of them needed a separate human answer and the ones nobody answered hung
// at zero tokens forever. They now share a single request: one answer frees them all.
it.instance(
  "ask - identical concurrent asks collapse to ONE request that one answer releases",
  () =>
    Effect.gen(function* () {
      const fibers = yield* Effect.all(
        [0, 1, 2].map(() =>
          ask({
            sessionID: SessionID.make("session_swarm"),
            permission: "bash",
            patterns: ["ls"],
            metadata: {},
            always: [],
      ...
      yield* waitForPending(1)
      yield* Effect.sleep("100 millis") // let all three park before we assert the count
      const pending = yield* list()
      expect(pending).toHaveLength(1) // three asks, ONE request (was three)

      // "once" approves nothing, so the always-sweep cannot be what releases the
      // siblings — only the shared deferred can.
...
it.instance(
  "ask - a DIFFERENT session's identical ask is never merged in",
  () =>
    Effect.gen(function* () {
      const own = { permission: "bash", patterns: ["ls"], metadata: {}, always: [], ruleset: [] } as const
      const a = yield* ask({ ...own, sessionID: SessionID.make("session_one") }).pipe(Effect.forkScoped)
      const b = yield* ask({ ...own, sessionID: SessionID.make("session_two") }).pipe(Effect.forkScoped)

      const pending = yield* waitForPending(2)
      expect(new Set(pending.map((item) => item.sessionID)).size).toBe(2)

      yield* reply({ requestID: pending[0]!.id, reply: "once" })
      expect(yield* list()).toHaveLength(1) // the other session is still waiting
...
// ---------------------------------------------------------------------------
// A SUB-AGENT's ask has a deadline; a main session's does not.
//
// The failure this closes: a sub-agent session has no window of its own. When
// the shell cannot surface its ask — an ACP client with no registered ancestor,
// a headless run, a client that never wired `requestPermission` — the deferred
// is never replied to. The sub-agent's tool call sits `running` for the life of
// the session and the PARENT's `task` call hangs behind it, showing a spinner
// and no reason. A refusal is an answer the agent can report; silence is not.
// ---------------------------------------------------------------------------

/** One second instead of the shipped 300, so the test costs a second. */
const fastTimeout = { experimental: { subagent_permission_timeout_seconds: 1 } }

it.instance(
  "ask - a SUB-AGENT's unanswered request is refused, and says why",
  () =>
    Effect.gen(function* () {
      const err = yield* fail(
        ask({
      ...
      // "The user rejected permission" would be a lie — nobody was asked, and
      // it is the lie that sends the agent back to retry the same call.
      expect((err as PermissionV1.RejectedError).message).toContain("nobody was watching this sub-agent's session")
      expect((err as PermissionV1.RejectedError).message).toContain('"bash"')
      expect((err as PermissionV1.RejectedError).message).toContain("1s")
    }),
  { git: true, config: fastTimeout },
)

it.instance(
  "ask - a MAIN session's request is NEVER timed out",
  () =>
    Effect.gen(function* () {
      // The user is looking at the bar. Waiting is correct: the answer is
      // coming when they get to it, and a self-cancelling prompt would be a
      // permission that granted itself a refusal behind their back.
      const fiber = yield* ask({
        sessionID: SessionID.make("session_main"),
        permission: "bash",
        patterns: ["ls"],
        metadata: {},
        always: [],
...
it.instance(
  "ask - 0 turns the sub-agent deadline off",
  () =>
    Effect.gen(function* () {
      const fiber = yield* ask({
        sessionID: SessionID.make("session_child"),
