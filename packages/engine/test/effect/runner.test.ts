...
describe("Runner", () => {
  ...
  it.live(
    "onJoin fires exactly once per discarded caller, and never on the one that starts the run",
    Effect.gen(function* () {
      const s = yield* Scope.Scope
      const joins = yield* Ref.make(0)
      const runner = Runner.make<string>(s, { onJoin: Ref.update(joins, (n) => n + 1) })

      const held = yield* Deferred.make<void>()
      const first = Deferred.await(held).pipe(Effect.as("first-result"))
      const fiber = yield* runner.ensureRunning(first).pipe(Effect.forkChild)
      yield* waitForState(runner, "Running")
      // The caller that STARTED the run has nothing queued behind it.
      expect(yield* Ref.get(joins)).toBe(0)

      const second = yield* runner.ensureRunning(Effect.succeed("second-result")).pipe(Effect.forkChild)
      const third = yield* runner.ensureRunning(Effect.succeed("third-result")).pipe(Effect.forkChild)
      yield* Effect.sleep("30 millis")
      // Two prompts arrived mid-run; both had their own work thrown away, so
      // both must be counted. A signal that fires once for "somebody joined"
      // cannot tell one lost message from five.
      expect(yield* Ref.get(joins)).toBe(2)

      yield* Deferred.succeed(held, undefined)
      expect(yield* Fiber.join(fiber)).toBe("first-result")
      // The proof that these two really were discarded: they got the FIRST
      // caller's answer, not their own.
      expect(yield* Fiber.join(second)).toBe("first-result")
      expect(yield* Fiber.join(third)).toBe("first-result")
    }),
  )

  it.live(
    "onJoin does NOT fire for a prompt behind a shell: that work is deferred, not discarded",
    Effect.gen(function* () {
      const s = yield* Scope.Scope
      const joins = yield* Ref.make(0)
      const runner = Runner.make<string>(s, { onJoin: Ref.update(joins, (n) => n + 1) })
      ...
      const held = yield* Deferred.make<void>()
      const shell = yield* runner.startShell(Deferred.await(held).pipe(Effect.as("shell"))).pipe(Effect.forkChild)
      yield* waitForState(runner, "Shell")

      const queued = yield* runner.ensureRunning(Effect.succeed("mine")).pipe(Effect.forkChild)
      yield* waitForState(runner, "ShellThenRun")
      expect(yield* Ref.get(joins)).toBe(0)

      yield* Deferred.succeed(held, undefined)
      yield* Fiber.join(shell)
      // It ran its OWN work after the shell finished - nothing was lost.
      expect(yield* Fiber.join(queued)).toBe("mine")
      expect(yield* Ref.get(joins)).toBe(0)
    }),
  )
