...
describe("acp session state", () => {
  ...
  // The per-chat SUB-AGENT override is a SECOND model on the session: what the
  // children run on, not what the chat runs on. The two must move apart.
  sessionTest.effect("keeps the sub-agent override independent of the chat's own model", () =>
    Effect.gen(function* () {
      yield* ACPSession.Service.use((session) =>
        session.load({ id: "ses_sub", cwd: "/workspace", model: model("anthropic", "claude-sonnet") }),
      )

      yield* ACPSession.Service.use((session) => session.setSubagentModel("ses_sub", model("openai", "gpt-5-mini")))
      expect(yield* ACPSession.Service.use((session) => session.getSubagentModel("ses_sub"))).toEqual(
        model("openai", "gpt-5-mini"),
      )
      expect(yield* ACPSession.Service.use((session) => session.getModel("ses_sub"))).toEqual(
        model("anthropic", "claude-sonnet"),
      )

      // Switching the CHAT's model must not drag the children with it...
      yield* ACPSession.Service.use((session) => session.setModel("ses_sub", model("anthropic", "claude-opus")))
      expect(yield* ACPSession.Service.use((session) => session.getSubagentModel("ses_sub"))).toEqual(
        model("openai", "gpt-5-mini"),
      )

      // ...and clearing the override must not disturb the chat.
      yield* ACPSession.Service.use((session) => session.setSubagentModel("ses_sub", undefined))
      expect(yield* ACPSession.Service.use((session) => session.getSubagentModel("ses_sub"))).toBeUndefined()
      expect(yield* ACPSession.Service.use((session) => session.getModel("ses_sub"))).toEqual(
