...
describe("acp.error", () => {
  ...
  test("a REFUSAL reaches the client as its own words, never as an internal error", () => {
    // The failure this exists to stop: every collab refusal used to be a
    // ServiceFailureError, so `Internal error: ` was glued onto a sentence
    // written for a human to act on. A refusal is not a fault in the engine -
    // the request was understood and declined - so it carries NO prefix at all.
    const requestError = ACPError.toRequestError(
      new ACPError.RefusalError({ service: "collab", safeMessage: "not in this collab: heron" }),
    )

    expect(requestError).toBeInstanceOf(RequestError)
    expect(requestError.message).toBe("not in this collab: heron")
    expect(requestError.message).not.toContain("Internal error")
    expect(requestError.message).not.toContain("Invalid params")
    expect(requestError.code).toBe(-32602)
    expect(requestError.data).toEqual({ service: "collab" })
  })
