...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.ORIGAMI_TEST ?? "")'), {
      env: {
        ORIGAMI_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "ORIGAMI_TEST_SHELL"], {
      shell: true,
      env: {
        ORIGAMI_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("ORIGAMI_TEST_SHELL=ok")
  })
