...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "origami.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "origami.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "origami.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "origami.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["origami.json", "origami.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "origami.json"),
        path.join(project, "origami.jsonc"),
      ])
    })
  })
  ...
  describe("resolve()", () => {
    ...
    test.skipIf(!canSymlink)("resolves symlinked directory to canonical path", async () => {
      await using tmp = await tmpdir()
    ...
    test.skipIf(!canSymlink)("throws ELOOP on symlink cycle", async () => {
      await using tmp = await tmpdir()
