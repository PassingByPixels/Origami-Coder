    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".origami/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
// Owner sessions, Origami Coder 0.3.86, 2026-08-15 (ChatGPT-OAuth / codex):
// "[!ERROR] prompt failed: Internal error: [ObjectParam] [input[5].arguments]
// [missing_required_parameter] Missing required parameter:
// 'input[5].arguments'." on the turn after the engine's own `tool_search`
// tool ran. Root cause: @ai-sdk/openai's Responses converter (shared by
// @ai-sdk/azure and @ai-sdk/amazon-bedrock/mantle, which both
// `require("@ai-sdk/openai/internal")`) special-cases a tool literally named
// "tool_search" as OPENAI'S OWN hosted tool on replay, regardless of which
// tool actually owns that name. Origami's deferred-tool-catalog tool
// (tool/tool-search.ts) collides on the name, so its replayed call gets
// parsed against OpenAI's hosted-tool schema, silently drops to
// `arguments: undefined`, and JSON.stringify omits the key on the wire.
describe("ProviderTransform.message - tool_search wire-name collision", () => {
  const openaiModel = {
    id: "openai/gpt-5.5",
    providerID: "openai",
  ...
  const toolCallAndResult = (toolName: string) =>
    [
      {
        role: "assistant",
        content: [{ type: "tool-call", toolCallId: "call_1", toolName, input: { query: "webfetch", limit: 5 } }],
      },
      {
        role: "tool",
        content: [{ type: "tool-result", toolCallId: "call_1", toolName, output: { type: "text", value: "ok" } }],
      },
    ] as any[]

  test("renames a replayed tool_search call and its result for the OpenAI SDK", () => {
    const result = ProviderTransform.message(toolCallAndResult("tool_search"), openaiModel, {}) as any[]

    expect(result[0].content[0].toolName).toBe(ProviderTransform.TOOL_SEARCH_WIRE_ALIAS)
    expect(result[1].content[0].toolName).toBe(ProviderTransform.TOOL_SEARCH_WIRE_ALIAS)
  })

  test("renames for Azure and Bedrock Mantle too, since both reuse the same vendored converter", () => {
    const azureModel = { ...openaiModel, providerID: "azure", api: { ...openaiModel.api, npm: "@ai-sdk/azure" } }
    const mantleModel = {
      ...openaiModel,
      providerID: "amazon-bedrock",
      api: { id: "openai.gpt-5.5", url: "https://bedrock-mantle.example", npm: "@ai-sdk/amazon-bedrock/mantle" },
    }

    for (const model of [azureModel, mantleModel]) {
      const result = ProviderTransform.message(toolCallAndResult("tool_search"), model, {}) as any[]
      expect(result[0].content[0].toolName).toBe(ProviderTransform.TOOL_SEARCH_WIRE_ALIAS)
      expect(result[1].content[0].toolName).toBe(ProviderTransform.TOOL_SEARCH_WIRE_ALIAS)
    }
  })

  test("leaves tool_search untouched for SDKs that don't share the vendored converter", () => {
    const anthropicModel = {
      ...openaiModel,
      providerID: "anthropic",
    ...
    const result = ProviderTransform.message(toolCallAndResult("tool_search"), anthropicModel, {}) as any[]

    expect(result[0].content[0].toolName).toBe("tool_search")
    expect(result[1].content[0].toolName).toBe("tool_search")
  })

  test("never touches a differently-named tool call, even on the OpenAI SDK", () => {
    const result = ProviderTransform.message(toolCallAndResult("read"), openaiModel, {}) as any[]

    expect(result[0].content[0].toolName).toBe("read")
    expect(result[1].content[0].toolName).toBe("read")
  })
})

describe("ProviderTransform.renameCollidingTool - tools map", () => {
  const openaiModel = {
    id: "openai/gpt-5.5",
    providerID: "openai",
    api: { id: "gpt-5.5", url: "https://api.openai.com", npm: "@ai-sdk/openai" },
  ...
  test("moves the tool_search entry to the wire alias for the OpenAI SDK", () => {
    const toolSearchTool = { description: "search deferred tools" }
    const tools = { read: { description: "read" }, tool_search: toolSearchTool }

    const result = ProviderTransform.renameCollidingTool(tools, openaiModel)

    expect(result["tool_search"]).toBeUndefined()
    expect(result[ProviderTransform.TOOL_SEARCH_WIRE_ALIAS]).toBe(toolSearchTool)
    expect(result["read"]).toBe(tools["read"])
  })

  test("leaves the tools map untouched for SDKs that don't share the vendored converter", () => {
    const tools = { read: { description: "read" }, tool_search: { description: "search deferred tools" } }

    const result = ProviderTransform.renameCollidingTool(tools, anthropicModel)

    expect(result).toEqual(tools)
  })

  test("is a no-op when tool_search is not offered this turn", () => {
    const tools = { read: { description: "read" } }
...
// origami_change: the OpenAI-family "declare then stop" arc.
//
// A gpt-5.1-and-later endpoint defaults to reasoning effort `none`, and a
// no-reasoning turn answers in one shot: it says what it is about to do and
// then ends, with no tool call. So the effort must be sent EXPLICITLY, in the
// spelling the endpoint on the other side of the request actually reads.
//
// Hosted OpenAI reads `reasoningEffort` (the SDK renames it). OpenRouter and
// LLM Gateway read `reasoning: { effort }` and spread every other key of the
// options record into the body verbatim, so `reasoningEffort` reaches them as
// an unknown field and changes nothing. `test/provider/openrouter-body.test.ts`
// proves that spread against the real vendor package; these tests fix which
// options the engine hands it.
describe("ProviderTransform - reasoning effort is an explicit, overridable default", () => {
  const sessionID = "session-effort"

  const gatewayModel = (apiId: string, providerID = "openrouter", npm = "@openrouter/ai-sdk-provider") =>
    ({
      id: `${providerID}/${apiId}`,
      providerID,
      api: { id: apiId, url: "https://openrouter.ai/api/v1", npm },
      name: apiId,
      capabilities: {
        temperature: true,
        reasoning: true,
  ...
  const hostedModel = (apiId: string) => {
    const model = gatewayModel(apiId, "openai", "@ai-sdk/openai")
    model.api.url = "https://api.openai.com/v1"
    return model
  }

  const prepared = (model: any, extra: { agent?: Record<string, any>; variant?: string } = {}) =>
    Effect.runPromise(
      LLMRequestPrep.prepare({
        user: {
          id: "msg_user-effort",
          ...
          time: { created: Date.now() },
          agent: "test",
          model: { providerID: model.providerID, modelID: model.id, variant: extra.variant },
        } as any,
        sessionID,
        model,
        agent: { name: "test", mode: "primary", options: extra.agent ?? {}, permission: [] } as any,
        system: [],
        messages: [{ role: "user", content: "Hello" }],
        tools: {},
  ...
  test("an openrouter gpt-5 model is sent the effort in the gateway's own shape", () => {
    const result = ProviderTransform.options({ model: gatewayModel("openai/gpt-5.1"), sessionID })
    expect(result.reasoning).toEqual({ effort: "medium" })
  })

  test("llmgateway gets the same shape as openrouter", () => {
    const model = gatewayModel("openai/gpt-5.1", "llmgateway", "@llmgateway/ai-sdk-provider")
    const result = ProviderTransform.options({ model, sessionID })
    expect(result.reasoning).toEqual({ effort: "medium" })
  })

  test("the gpt-5 ids that accept no medium effort are left alone on a gateway", () => {
    // Which ids these are is not a guess: this file's own effort tables say it.
    // Unversioned chat (`gpt-5-chat`) exposes NO efforts at all
    // (`gpt5ChatReasoningEfforts` returns []), and unversioned pro exposes only
    // "high" (`OPENAI_GPT5_PRO_EFFORTS`). Sending "medium" to either is a
    // request the endpoint rejects.
    for (const id of ["openai/gpt-5-chat", "openai/gpt-5-pro"])
      expect(ProviderTransform.options({ model: gatewayModel(id), sessionID }).reasoning, id).toBeUndefined()
  })

  test("versioned chat and pro ids DO get the effort, because they accept medium", () => {
    // The counterpart to the test above, and the reason the exclusion is two
    // exact ids rather than "-chat"/"-pro" anywhere in the id: versioned chat
    // accepts ["medium"] (`OPENAI_GPT5_CHAT_EFFORTS`) and versioned pro accepts
    // ["medium","high","xhigh"] (`OPENAI_GPT5_PRO_2_PLUS_EFFORTS`). Widening
    // the exclusion would silently put both back on the endpoint default.
    for (const id of ["openai/gpt-5.1-chat", "openai/gpt-5.2-pro"])
      expect(ProviderTransform.options({ model: gatewayModel(id), sessionID }).reasoning, id).toEqual({
        effort: "medium",
      })
  })

  test("a non-gpt gateway model is untouched by the gpt-5 branch", () => {
    const model = gatewayModel("anthropic/claude-opus-4.5")
    expect(ProviderTransform.options({ model, sessionID }).reasoning).toBeUndefined()
  })

  test("the gemini-3 gateway effort still wins on a gemini id", () => {
    const model = gatewayModel("google/gemini-3-pro")
    expect(ProviderTransform.options({ model, sessionID }).reasoning).toEqual({ effort: "high" })
  })

  // --- The default is a DEFAULT. Three routes replace it, and all three go
  // through the same merge in session/llm/request.ts, so each is a live user
  // control rather than something that needs an engine rebuild.

  test("a model's configured options replace the shipped effort", async () => {
    const model = hostedModel("gpt-5.1")
    model.options = { reasoningEffort: "high" }
    const result = await prepared(model)
    expect(result.params.options.reasoningEffort).toBe("high")
  })

  test("an agent's options replace the shipped effort", async () => {
    const result = await prepared(hostedModel("gpt-5.1"), { agent: { reasoningEffort: "low" } })
    expect(result.params.options.reasoningEffort).toBe("low")
  })

  test("a selected variant replaces the gateway effort without leaving the default behind", async () => {
    const model = gatewayModel("openai/gpt-5.1")
    model.variants = { high: { reasoning: { effort: "high" } } }
    const result = await prepared(model, { variant: "high" })
    expect(result.params.options.reasoning).toEqual({ effort: "high" })
  })

  test("with no override the shipped effort survives the merge to the request", async () => {
    const result = await prepared(hostedModel("gpt-5.1"))
    expect(result.params.options.reasoningEffort).toBe("medium")
  })

  // --- smallOptions: the branch that turns reasoning OFF for a gateway model.
  // It is scoped to google ids, and the title/summary call is the only caller
  // (`input.small` in session/llm/request.ts). This is the guard that catches
  // it widening onto the family that cannot afford to lose reasoning.

  test("the gateway reasoning-off branch never applies to a gpt model", () => {
    for (const id of ["openai/gpt-5.1", "openai/gpt-5.2-codex", "openai/gpt-4o"])
      expect(ProviderTransform.smallOptions(gatewayModel(id)), id).not.toHaveProperty("reasoning")
  })

  test("the gateway reasoning-off branch still applies to a google id with no variants", () => {
