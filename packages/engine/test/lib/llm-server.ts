...
export class TestLLMServer extends Context.Service<TestLLMServer, TestLLMServer.Service>()("@test/LLMServer") {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      /**
       * Only entries queued with an explicit matcher, for requests the server
       * would otherwise answer by itself. Without the matcher requirement such a
       * request would consume the next unmatched reply, which belongs to the
       * turn the test is actually driving.
       */
      const pullMatched = (hit: Hit) => {
        const index = list.findIndex((entry) => entry.match?.(hit) === true)
        if (index === -1) return
        const first = list[index]
        list = [...list.slice(0, index), ...list.slice(index + 1)]
        return first.item
      ...
      const handle = Effect.fn("TestLLMServer.handle")(function* (mode: "chat" | "responses") {
        ...
        if (isTitleRequest(body)) {
          hits = [...hits, current]
          yield* notify()
          // A test may script the title turn explicitly — a binding that answers
          // 403, say. Anything else keeps the server answering it by itself, so
          // ordinary tests never have to queue a title reply.
