// Subprocess test harness for the origami CLI. Spawns the real binary against
...
//
// Configuration flows through origami's built-in test affordances:
//   - ORIGAMI_CONFIG_CONTENT      : provider config inline, no files to find
//   - ORIGAMI_TEST_HOME           : pins os.homedir() → tmpdir
//   - ORIGAMI_DISABLE_PROJECT_CONFIG : skip walking up for origami.json
//   - ORIGAMI_PURE                : skip external plugin discovery + install
//   - ORIGAMI_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `origami.run` is fully wired. The shape supports adding more
// builders (`origami.serve(opts)`, `origami.acp(opts)`, `origami.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of OrigamiCli.
...
const origamiRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(origamiRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    ...
    ORIGAMI_CONFIG_CONTENT: configJson,
    ORIGAMI_DISABLE_PROJECT_CONFIG: "1",
    ORIGAMI_PURE: "1",
    ORIGAMI_DISABLE_AUTOUPDATE: "1",
    ORIGAMI_DISABLE_AUTOCOMPACT: "1",
    ORIGAMI_DISABLE_MODELS_FETCH: "1",
    ORIGAMI_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `origami run`. New flags should
...
// `origami serve` is a long-lived process — it never exits on its own.
...
// `origami acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OrigamiCli = {
  ...
  // Spawn `origami serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `origami acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("origami.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          ORIGAMI_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("origami.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("origami.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "origami server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `origami serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("origami.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const origami: OrigamiCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, origami })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `origami.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "origami") {
...
// every call site. Use this for any test that needs the origami CLI fixture.
//
...
// resources (e.g. `origami.serve`) without an extra `Effect.scoped` wrapper —
