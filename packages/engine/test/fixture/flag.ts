...
/**
 * Scoped override for `Flag.ORIGAMI_WORKSPACE_ID`. Saves the previous value
...
export function withFixedWorkspaceID(id: WorkspaceV2.ID): Effect.Effect<void, never, Scope.Scope> {
  return Effect.gen(function* () {
    const previous = Flag.ORIGAMI_WORKSPACE_ID
    Flag.ORIGAMI_WORKSPACE_ID = id
    yield* Effect.addFinalizer(() =>
      Effect.sync(() => {
        Flag.ORIGAMI_WORKSPACE_ID = previous
      }),
    )
  })
}
