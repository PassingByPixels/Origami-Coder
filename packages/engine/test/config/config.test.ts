...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    if (request.url.includes(".well-known/origami")) {
...
const managedConfigDir = process.env.ORIGAMI_TEST_MANAGED_CONFIG_DIR!
const originalTestToken = process.env.TEST_TOKEN
const originalConsoleToken = process.env.ORIGAMI_CONSOLE_TOKEN
...
afterEach(async () => {
  ...
  if (originalConsoleToken === undefined) delete process.env.ORIGAMI_CONSOLE_TOKEN
  else process.env.ORIGAMI_CONSOLE_TOKEN = originalConsoleToken
  await clear(true)
})

const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(path.join(managedConfigDir, filename ?? "origami.json"), JSON.stringify(settings))

async function writeConfig(dir: string, config: object, name = "origami.json") {
  await Filesystem.write(path.join(dir, name), JSON.stringify(config))
}

const writeConfigEffect = (dir: string, config: object, name = "origami.json") =>
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local ? writeConfigEffect(path.join(directory, ".origami"), schemaConfig(input.local)) : undefined,
...
// SEEDS origami.json, NOT origami.jsonc. The `.jsonc` candidate outranks
// `.json` in loadGlobal's merge below, and the VS Code extension's seven config
// writers only ever touch `origami.json` (they have no candidate search). So an
// empty `.jsonc` seed sat permanently at the TOP of the merge order and
// shadowed every write the Connections panel made — with no error on either
// side, because both halves succeeded at what they thought they were doing.
// Connections adversarial review 2026-08-15, finding 1.
it.effect("creates an empty global json config when no global configs exist", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.get().pipe(provideInstanceEffect(dir))

      const content = yield* FSUtil.use.readFileString(path.join(dir, "origami.json"))
      // This fork hosts no config JSON Schema, so the seeded file must not claim one.
      expect(JSON.parse(content)).toEqual({})
      expect(content).not.toContain("$schema")
      // The file that used to be seeded here must NOT appear: it would outrank
      // this one forever.
      expect(yield* FSUtil.use.existsSafe(path.join(dir, "origami.jsonc"))).toBe(false)
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

// The other half of finding 1: the empty seeds ALREADY on disk. The owner's own
// machine carries a hand-renamed `origami.jsonc.disabled`, byte-exact for the
// seeded `{}` — this had already cost them once, and they fixed it by hand.
it.effect("folds an EMPTY origami.jsonc aside when it is shadowing a populated origami.json", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      yield* writeConfigEffect(dir, { model: "vllm/deepseek" }, "origami.json")
      yield* writeConfigEffect(dir, {}, "origami.jsonc")

      const config = yield* Config.use.get().pipe(provideInstanceEffect(dir))

      // The value the extension wrote is the value in force.
      expect(config.model).toBe("vllm/deepseek")
      // RENAMED, never deleted — a loader that removes files from a user's
      // config directory is not a trade worth making, even for two bytes.
      expect(yield* FSUtil.use.existsSafe(path.join(dir, "origami.jsonc"))).toBe(false)
      const aside = path.join(dir, `origami.jsonc.empty-seed-${new Date().toISOString().slice(0, 10)}`)
      expect(JSON.parse(yield* FSUtil.use.readFileString(aside))).toEqual({})
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

// The guard on the migration. Anything in the file is the USER's, and a
// deliberate `.jsonc` is a supported way to configure this engine — it must
// keep winning, exactly as before.
it.effect("leaves a NON-empty origami.jsonc alone, and it still wins the merge", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      yield* writeConfigEffect(dir, { model: "vllm/from-json" }, "origami.json")
      yield* FSUtil.use.writeWithDirs(
        path.join(dir, "origami.jsonc"),
        '{\n  // the one I actually want\n  "model": "vllm/from-jsonc"\n}\n',
      )

      const config = yield* Config.use.get().pipe(provideInstanceEffect(dir))

      expect(config.model).toBe("vllm/from-jsonc")
      expect(yield* FSUtil.use.existsSafe(path.join(dir, "origami.jsonc"))).toBe(true)
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

// The Zen provider-id fold, end to end through the LOADER — the unit cases for
// the rewrite itself live in test/origami/zen-provider-migrate.test.ts. What is
// proven here is the wiring: a config the old sidebar wrote comes back off the
// loader under the id the engine's Zen feature gates actually match.
it.effect("folds a legacy provider.opencode-zen block over to provider.opencode on load", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      yield* writeConfigEffect(dir, { provider: { "opencode-zen": { options: { apiKey: "sk-zen" } } } })

      const config = yield* Config.use.get().pipe(provideInstanceEffect(dir))

      expect(config.provider?.["opencode"]).toBeDefined()
      expect(config.provider?.["opencode"]?.options?.apiKey).toBe("sk-zen")
      expect(config.provider?.["opencode-zen"]).toBeUndefined()
      // the user's original file is recoverable
      expect(yield* FSUtil.use.existsSafe(path.join(dir, "origami.json.zen-provider.bak"))).toBe(true)
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

// An explicit `opencode` block is the user's own decision and outranks the fold.
it.effect("leaves both blocks alone when the user already has a provider.opencode", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      yield* writeConfigEffect(dir, {
        provider: { "opencode-zen": { options: { apiKey: "sk-old" } }, opencode: { options: { apiKey: "sk-new" } } },
      })

      const config = yield* Config.use.get().pipe(provideInstanceEffect(dir))

      expect(config.provider?.["opencode"]?.options?.apiKey).toBe("sk-new")
      expect(config.provider?.["opencode-zen"]).toBeDefined()
      expect(yield* FSUtil.use.existsSafe(path.join(dir, "origami.json.zen-provider.bak"))).toBe(false)
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)

it.effect("does not create global config when ORIGAMI_CONFIG_DIR is set", () =>
  Effect.gen(function* () {
...
it.effect("updates global config and omits empty shell key in json", () =>
  withGlobalConfig({ config: { shell: "bash" } }, ({ dir }) =>
    Effect.gen(function* () {
      ...
      const writtenConfig = yield* FSUtil.use.readJson(path.join(dir, "origami.json"))
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig({ config: { shell: "bash", model: "test/model" }, name: "origami.jsonc" }, ({ dir }) =>
    Effect.gen(function* () {
      yield* Config.use.updateGlobal({ shell: "" })

      const file = path.join(dir, "origami.jsonc")
...
// Finding 33. The `.json` branch used to decode-then-rewrite, and Schema.Struct
// strips undeclared keys at every NESTED level (parse.ts's topLevelExtraKeys
// check only re-guards the top). So one `PUT /config` from an older engine
// against a config a newer extension had written silently dropped the nested
// field, and the toggle for it reverted with no explanation. Both branches
// patch the TEXT now, and text cannot drop what it does not touch.
it.effect("updateGlobal on a .json file keeps nested keys this build does not declare", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      const file = path.join(dir, "origami.json")
      yield* FSUtil.use.writeWithDirs(
        file,
        JSON.stringify(
          { model: "vllm/old", provider: { vllm: { models: { x: { name: "x", future_field: 42 } } } } },
          null,
          2,
        ),
      )

      yield* Config.use.updateGlobal({ model: "vllm/new" })

      const written = yield* FSUtil.use.readJson(file)
      expect(written).toMatchObject({
        model: "vllm/new",
        provider: { vllm: { models: { x: { future_field: 42 } } } },
      })
    }),
  ),
)

// Same change, second consequence: the engine has always PARSED origami.json as
// JSONC, so a user may legitimately have commented it — and a whole-file
// rewrite deleted their comments on the next write.
it.effect("updateGlobal on a .json file preserves the user's comments", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      const file = path.join(dir, "origami.json")
      ...
      expect(written).toContain("// Spark 2 is the second DGX box")
      expect(written).toContain("vllm/new")
    }),
  ),
)

// Finding 23. getLiveAgentSampling reads every candidate and merges later-wins,
// exactly like loadGlobal — but its list was the exact REVERSE of loadGlobal's,
// so legacy config.json outranked origami.jsonc here while origami.jsonc
// outranked it everywhere else. A user who set temperature in the .jsonc got
// the new value in the cached config and the STALE one on the wire.
it.effect("getLiveAgentSampling resolves the same file precedence as the cached config", () =>
  withGlobalConfig({}, ({ dir }) =>
    Effect.gen(function* () {
      yield* writeConfigEffect(dir, { agent: { build: { temperature: 0.1 } } }, "config.json")
      yield* writeConfigEffect(dir, { agent: { build: { temperature: 0.5 } } }, "origami.json")
      yield* FSUtil.use.writeWithDirs(
        path.join(dir, "origami.jsonc"),
        '{\n  // the value I actually set\n  "agent": { "build": { "temperature": 0.9, "top_p": 0.8 } }\n}\n',
      )

      const cached = yield* Config.use.get().pipe(provideInstanceEffect(dir))
      const live = yield* Config.use.getLiveAgentSampling("build")

      // The same file has to win in both, or the wire disagrees with the config.
      expect(cached.agent?.build?.temperature).toBe(0.9)
      expect(live.temperature).toBe(0.9)
      // …and the candidate the old list put FIRST is the one JSON.parse could
      // not read at all once it had a comment in it.
      expect(live.topP).toBe(0.8)
    }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
  ),
)
...
it.instance("ignores legacy tui keys in origami config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
...
it.instance("loads JSONC config file", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, "origami.jsonc"),
      `{
        // This is a comment
        "model": "test/model",
        "username": "testuser"
...
it.instance("leaves the config file untouched when substituting env variables", () =>
  withProcessEnv(
    "PRESERVE_VAR",
    "secret_value",
    Effect.gen(function* () {
      ...
      yield* FSUtil.use.writeWithDirs(path.join(test.directory, "origami.json"), original)
      const config = yield* Config.use.get()
      expect(config.username).toBe("secret_value")

      // Loading must never rewrite the file: a rewrite would bake the resolved
      // secret into it. Nothing is added on load, not even a $schema key.
      const content = yield* FSUtil.use.readFileString(path.join(test.directory, "origami.json"))
...
const accountTokenIt = configIt({
  account: Layer.mock(Account.Service)({
    ...
    config: () =>
      Effect.succeed(
        Option.some({
          provider: { opencode: { options: { apiKey: "{env:ORIGAMI_CONSOLE_TOKEN}" } } },
        }),
      ),
...
it.instance("throws error for invalid JSON", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(path.join(test.directory, "origami.json"), "{ invalid json }")
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
it.instance("loads config from .origami directory", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "agent", "test.md"),
      `---
model: test/model
---
Test agent prompt`,
...
it.instance("agent markdown permission config preserves user key order", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "agent", "ordered.md"),
      `---
permission:
  bash: allow
  "*": deny
...
it.instance("loads agents from .origami/agents (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "agents", "helper.md"),
      `---
model: test/model
mode: subagent
---
Helper agent prompt`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "agents", "nested", "child.md"),
      `---
model: test/model
mode: subagent
---
...
it.instance("loads commands from .origami/command (singular)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "command", "hello.md"),
      `---
description: Test command
---
Hello from singular command`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "command", "nested", "child.md"),
      `---
...
it.instance("loads commands from .origami/commands (plural)", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "commands", "hello.md"),
      `---
description: Test command
---
Hello from plural commands`,
    ...
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "commands", "nested", "child.md"),
      `---
description: Nested command
---
Nested command template`,
...
it.effect("does not try to install dependencies in read-only ORIGAMI_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("ORIGAMI_CONFIG_DIR", readonly, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("ignores an inaccessible ORIGAMI_CONFIG_DIR", () =>
  Effect.gen(function* () {
    if (process.platform === "win32") return
    ...
    yield* withProcessEnv("ORIGAMI_CONFIG_DIR", configDir, Config.use.get().pipe(provideInstanceEffect(dir)))
  }).pipe(Effect.provide(testInstanceStoreLayer), Effect.provide(LayerNode.compile(CrossSpawnSpawner.node))),
)

it.effect("creates a missing ORIGAMI_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
...
it.effect("installs dependencies in writable ORIGAMI_CONFIG_DIR", () =>
  Effect.gen(function* () {
    const dir = yield* tmpdirScoped()
    const configDir = path.join(dir, "configdir")
    yield* FSUtil.use.ensureDir(configDir)
...
it.instance("does not error when only custom agent is a subagent", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, ".origami", "agent", "helper.md"),
      `---
model: test/model
mode: subagent
---
...
// Note: preload.ts sets ORIGAMI_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
it.instance("managed jsonc settings override managed json settings", () =>
  Effect.gen(function* () {
    ...
    yield* writeManagedSettingsEffect({ model: "managed/jsonc" }, "origami.jsonc")
...
it.instance("local .origami config can override MCP from project config", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance
    yield* writeConfigEffect(test.directory, {
      mcp: {
    ...
    yield* FSUtil.use.ensureDir(path.join(test.directory, ".origami"))
    yield* writeConfigEffect(
      path.join(test.directory, ".origami"),
      {
        mcp: {
          docs: {
            type: "remote",
...
remoteProjectOverride.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remoteProjectOverride.seen.wellKnown).toBe("https://example.com/.well-known/origami")
...
trailingSlashWellKnown.it.instance("wellknown URL with trailing slash is normalized", () =>
  Effect.gen(function* () {
    ...
    expect(trailingSlashWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/origami")
  }),
)
      ...
      () =>
        Config.Service.use((svc) =>
          Effect.gen(function* () {
            ...
            expect(fetchedUrl).toBe(`${server.url.origin}/.well-known/origami`)
            expect(config.mcp?.jira?.enabled).toBe(true)
          }),
        ),
      { git: true },
...
const templatedHeaderWellKnown = wellKnown({
  remoteConfig: {
    url: "https://config.example.com/origami.json",
...
templatedHeaderWellKnown.it.instance("wellknown remote_config supports templated env vars in headers", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(templatedHeaderWellKnown.seen.wellKnown).toBe("https://example.com/.well-known/origami")
    expect(templatedHeaderWellKnown.seen.remote).toBe("https://config.example.com/origami.json")
...
const remotePrecedenceWellKnown = wellKnown({
  ...
  remoteConfig: { url: "https://config.example.com/{env:TEST_TOKEN}/origami.json" },
  remote: {
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get()
      expect(remotePrecedenceWellKnown.seen.remote).toBe("https://config.example.com/test-token/origami.json")
      expect(config.mcp?.confluence?.enabled).toBe(true)
    }),
)
...
const nullConfigWellKnown = wellKnown({
  wellKnown: {
    config: null,
    remote_config: { url: "https://config.example.com/origami.json" },
  },
  remote: {
    mcp: { confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true } },
  },
...
nullConfigWellKnown.it.instance("wellknown config null is treated as absent", () =>
  Effect.gen(function* () {
    const config = yield* Config.use.get()
    expect(nullConfigWellKnown.seen.remote).toBe("https://config.example.com/origami.json")
    expect(config.mcp?.confluence?.enabled).toBe(true)
  }),
)

const invalidRemoteWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/origami.json" },
...
invalidRemoteWellKnown.it.instance("wellknown remote_config rejects non-object config responses", () =>
  Effect.gen(function* () {
    const exit = yield* Config.use.get().pipe(Effect.exit)
    expect(invalidRemoteWellKnown.seen.remote).toBe("https://config.example.com/origami.json")
    expect(Exit.isFailure(exit)).toBe(true)
  }),
)
...
loginPageWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const exit = yield* Config.use.get().pipe(Effect.exit)
      expect(loginPageWellKnown.seen.remote).toBe("https://config.example.com/origami.json")
      expect(Exit.isFailure(exit)).toBe(true)
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    const file = path.join(tmp.path, "origami.json")
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-origami@2.4.3", file)).toBe("oh-my-origami@2.4.3")
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("keeps path plugins separate from package plugins", () => {
    const plugins = ["oh-my-origami@2.4.3", "file:///project/.origami/plugin/oh-my-origami.js"]
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = ["file:///project/.origami/plugin/demo.ts", "file:///project/.origami/plugin/demo.ts"]

    const result = dedupe(plugins)

    expect(result).toEqual(["file:///project/.origami/plugin/demo.ts"])
  })
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".origami", "plugin", "my-plugin.js"),
          "export default {}",
        )
...
describe("ORIGAMI_DISABLE_PROJECT_CONFIG", () => {
  it.instance(
  ...
  it.instance("skips project .origami/ directories when flag is set", () =>
    withProcessEnv(
      "ORIGAMI_DISABLE_PROJECT_CONFIG",
      "true",
      Effect.gen(function* () {
        const test = yield* TestInstance
        yield* FSUtil.use.writeWithDirs(
          path.join(test.directory, ".origami", "command", "test-cmd.md"),
  ...
  it.instance(
    ...
    () =>
      withProcessEnvs(
        { ORIGAMI_CONFIG_DIR: undefined, ORIGAMI_DISABLE_PROJECT_CONFIG: "true" },
        Effect.gen(function* () {
          const test = yield* TestInstance
  ...
  it.instance(
    "ORIGAMI_CONFIG_DIR still works when flag is set",
    () =>
      Effect.gen(function* () {
        const configDir = yield* tmpdirScoped({ config: { model: "configdir/model" } })
        yield* withProcessEnvs(
          { ORIGAMI_DISABLE_PROJECT_CONFIG: "true", ORIGAMI_CONFIG_DIR: configDir },
          Effect.gen(function* () {
            const config = yield* Config.use.get()
...
// Regression for #28206: malformed ORIGAMI_PERMISSION JSON used to crash
...
describe("ORIGAMI_PERMISSION env var", () => {
  it.instance("does not crash when ORIGAMI_PERMISSION contains invalid JSON", () =>
    withProcessEnv(
      "ORIGAMI_PERMISSION",
      "{invalid",
      Effect.gen(function* () {
...
describe("ORIGAMI_CONFIG_CONTENT token substitution", () => {
  it.instance("substitutes {env:} tokens in ORIGAMI_CONFIG_CONTENT", () =>
    withProcessEnv(
      "TEST_CONFIG_VAR",
      "test_api_key_12345",
      withProcessEnv(
        "ORIGAMI_CONFIG_CONTENT",
        JSON.stringify({
  ...
  it.instance("substitutes {file:} tokens in ORIGAMI_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
  ...
  const config = ConfigParse.schema(
    ...
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(
        JSON.stringify({
          PayloadDisplayName: "Origami Managed",
          PayloadIdentifier: "ai.origami.managed.test",
          PayloadType: "ai.origami.managed",
...
test("parseManagedPlist handles empty config", async () => {
  const config = ConfigParse.schema(
    ConfigV1.Info,
    ConfigParse.jsonc(
      await ConfigManaged.parseManagedPlist(JSON.stringify({ $schema: "https://example.com/config.json" })),
      "test:mobileconfig",
    ),
    "test:mobileconfig",
  )
  expect(config.$schema).toBe("https://example.com/config.json")
})
