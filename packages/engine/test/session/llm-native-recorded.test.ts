...
import { HttpRecorder } from "@origami/http-recorder"
...
import { LLMEvent, LLMResponse } from "@origami/llm"
import { RequestExecutor } from "@origami/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.ORIGAMI_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["origami", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ORIGAMI_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("ORIGAMI_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["origami", "native", "oauth", "tool-loop"],
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["origami", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.ORIGAMI_RECORD_CONSOLE_TOKEN && process.env.ORIGAMI_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.opencode,
        name: "OpenCode Zen",
        env: ["ORIGAMI_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.ORIGAMI_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.ORIGAMI_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.ORIGAMI_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["origami", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("ORIGAMI_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("ORIGAMI_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("ORIGAMI_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the origami LLM stack remain real.
  const metadata = {
