...
/**
 * The exact check the AI SDK runs on every request: `standardizePrompt` validates
 * the array with `z.array(modelMessageSchema)` and throws AI_InvalidPromptError
 * ("The messages do not match the ModelMessage[] schema.") when it fails. A
 * failure sourced from STORED history is unrecoverable by retrying: every later
 * turn in that session replays the same bad part and dies the same way.
 */
function expectValidModelMessages(messages: unknown[]) {
...
describe("session.message-v2.toModelMessage", () => {
  ...
  // An engine notice (stream-drop retry here) is stored as an assistant text part
  // whose metadata is a FLAT string record - SessionProcessor.notice types it
  // `Record<string, string>`. providerOptions must be a TWO-level
  // Record<providerId, Record<string, JSONValue>>, so replaying the marker
  // verbatim invalidates the whole messages array and bricks the session.
  test("drops flat engine markers from replayed provider metadata", async () => {
    const assistantID = "m-assistant-retry-notice"
    const input: SessionV1.WithParts[] = [
      {
        info: assistantInfo(assistantID, "m-parent"),
        parts: [
          {
            ...basePart(assistantID, "p1"),
            type: "text",
            text: "Stream dropped (The provider stream ended with no finish reason.) - retrying, attempt 1 of 3.",
            metadata: { origami_retry: "1" },
          },
        ] as SessionV1.Part[],
      },
    ]
    ...
    const text = (result[0].content as any[]).find((p) => p.type === "text")
    expect(text.providerOptions).toBeUndefined()
    expectValidModelMessages(result)
  })

  test("keeps provider-shaped metadata on replayed reasoning", async () => {
    const assistantID = "m-assistant-signed"
    const input: SessionV1.WithParts[] = [
      {
        info: assistantInfo(assistantID, "m-parent"),
        parts: [
    ...
    const reasoning = (result[0].content as any[]).find((p) => p.type === "reasoning")
    expect(reasoning.providerOptions).toStrictEqual({ anthropic: { signature: "sig" } })
    expectValidModelMessages(result)
  })

  test("keeps only the provider-shaped entry when metadata mixes both", async () => {
        ...
        parts: [
          {
            ...
            type: "text",
            text: "answer",
            metadata: { origami_retry: "1", anthropic: { signature: "s" } },
          },
        ] as SessionV1.Part[],
      },
    ]
    ...
    const text = (result[0].content as any[]).find((p) => p.type === "text")
    expect(text.providerOptions).toStrictEqual({ anthropic: { signature: "s" } })
    expectValidModelMessages(result)
  })
})
...
describe("session.message-v2.fromError", () => {
  ...
  test("classifies ZlibError from fetch as retryable APIError", () => {
    const zlibError = new Error(
      'ZlibError fetching "https://origami.cloudflare.dev/anthropic/messages". For more information, pass `verbose: true` in the second argument to fetch()',
    )
...
describe("session.message-v2.latest", () => {
  ...
  // origami_change: the invariant that makes compaction safe for replay to a
  // reasoning model. A reasoning item has to stay with the tool call it
  // produced; keep one and drop the other and the next turn is rejected
  // (vercel/ai#11036). `filterCompacted` is allowed to drop and REORDER whole
  // messages - the slice below is the one place it does - but it must never
  // reach inside `parts`. Nothing asserted that, so the safety of the whole
  // compaction path rested on reading the code.
  test("compaction reorders messages but never reaches inside a message's parts", () => {
    const REASONING_ASSISTANT = MessageID.make("msg_007")
    const parts = [
      { ...basePart(REASONING_ASSISTANT, "p1"), type: "reasoning", text: "thinking" },
      {
        ...basePart(REASONING_ASSISTANT, "p2"),
        type: "tool",
        tool: "bash",
        callID: "call_1",
        state: { status: "completed", input: {}, output: "ok", title: "bash", metadata: {}, time: { start: 0, end: 1 } },
      },
    ] as SessionV1.Part[]
    const reasoningAssistant: SessionV1.WithParts = {
      info: { ...assistantInfo(REASONING_ASSISTANT, TAIL_USER), finish: "tool-calls" } as SessionV1.Assistant,
      parts,
    }
    ...
    const survivor = filtered.find((msg) => msg.info.id === REASONING_ASSISTANT)
    expect(survivor, "the retained tail assistant was dropped altogether").toBeDefined()
    expect(survivor!.parts.map((part) => part.type)).toEqual(["reasoning", "tool"])
    // Identity, not equality: the array is the SAME object it went in as, which
    // is what proves no part-level rebuild happened on the way through.
    expect(survivor!.parts).toBe(parts)
  })
