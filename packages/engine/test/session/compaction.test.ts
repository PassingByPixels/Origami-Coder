...
describe("session.compaction.isOverflow", () => {
  ...
  // Per-chat auto-compaction TRIGGER override (t-kgsdsw). Once a sessionID is
  // passed, a threshold on that session's row is authoritative over the
  // cfg-derived reserve these tests exercise above.
  it.live(
    "a percent threshold on the session row triggers well before the cfg-derived reserve would",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const ssn = yield* SessionNs.Service
        ...
        const info = yield* ssn.create({
          metadata: SessionNs.withCompactionThreshold(undefined, { kind: "percent", value: 0.5 }),
        })
        // 55K tokens: well under the cfg default (usable = 68K) but over the
        // 50%-of-context override (usable = 50K).
        const tokens = { input: 50_000, output: 5_000, reasoning: 0, cache: { read: 0, write: 0 } }
        expect(yield* compact.isOverflow({ tokens, model, sessionID: info.id })).toBe(true)
        expect(yield* compact.isOverflow({ tokens, model })).toBe(false)
      }),
    ),
  )
  ...
  it.live(
    "an absolute token threshold on the session row is used verbatim, ignoring the cfg reserve",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const ssn = yield* SessionNs.Service
        const model = createModel({ context: 200_000, output: 32_000 })
        const info = yield* ssn.create({
          metadata: SessionNs.withCompactionThreshold(undefined, { kind: "tokens", value: 30_000 }),
        })
        const tokens = { input: 25_000, output: 4_000, reasoning: 0, cache: { read: 2_000, write: 0 } }
        // count = 31K >= 30K override -> true, well under the 168K cfg-derived usable.
        expect(yield* compact.isOverflow({ tokens, model, sessionID: info.id })).toBe(true)
      }),
    ),
  )
  ...
  it.live(
    "a session with no threshold override falls back to the cfg-derived reserve unchanged",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const ssn = yield* SessionNs.Service
  ...
  it.live(
    "a sessionID for a session that no longer exists falls back to the cfg-derived reserve rather than failing",
    provideTmpdirInstance(() =>
      Effect.gen(function* () {
        const compact = yield* SessionCompaction.Service
        const model = createModel({ context: 100_000, output: 32_000 })
        const tokens = { input: 75_000, output: 5_000, reasoning: 0, cache: { read: 0, write: 0 } }
        const ghost = "ses_does_not_exist" as SessionID
        expect(yield* compact.isOverflow({ tokens, model, sessionID: ghost })).toBe(true)
      }),
    ),
  )
})
...
describe("session.compaction.process", () => {
  ...
  itCompaction.instance(
    ...
    Effect.gen(function* () {
      ...
      // The hook suppressed the continuation, so the turn is OVER. Answering
      // "continue" left the caller to discover that for itself: the summary
      // satisfies the loop's exit gate, the loop breaks, and the session goes
      ...
      // One line, in the transcript, saying which of the two silences this is.
      const notice = all
        .flatMap((item) => item.parts)
        .find(
          (part): part is SessionV1.TextPart =>
            part.type === "text" && part.metadata?.["origami_compaction_stop"] === "autocontinue",
        )
      expect(notice?.text).toContain("autocontinue")
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        // The requirement is "the interrupt does not sit out the backoff":
        // rungs run 500ms-4s (stream-drop) and longer on the rate-limit
        // ladder. The bound must sit well under those while staying above
        // scheduler/GC noise - a 250ms bound flaked at 305-1264ms on a
        // loaded box while proving nothing extra.
        const exit = yield* Fiber.await(fiber).pipe(Effect.timeout("2500 millis"))
