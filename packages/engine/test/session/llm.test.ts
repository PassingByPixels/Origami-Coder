...
import { LLMClient, RequestExecutor } from "@origami/llm/route"
...
describe("session.llm.stream", () => {
  ...
  it.instance(
    "per-session temperature/top_p override the agent defaults in the built request",
    () =>
      // Per-chat sampling (SessionV1.User.temperature / topP, threaded from the
      // prompt input) must win over the agent's own temperature/topP in the request
      // sent to the provider. The config here has NO global agent sampling override,
      // so if the per-session merge were dropped the request would carry the agent's
      // 0.4 / 0.8 instead of the session's 1.3 / 0.25 — this test goes red.
      Effect.gen(function* () {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        const request = waitRequest(
          "/chat/completions",
        ...
        const sessionID = SessionID.make("session-test-per-session-sampling")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const user = {
          id: MessageID.make("msg_user-per-session-sampling"),
          sessionID,
          role: "user",
          time: { created: Date.now() },
          agent: agent.name,
        ...
        const capture = yield* Effect.promise(() => request)
        expect(capture.body.temperature).toBe(1.3)
        expect(capture.body.top_p).toBe(0.25)
      }),
    {
      config: () => ({
        enabled_providers: [vivgridFixture.providerID],
...
describe("session.llm.stream — held think-scanner text never outlives the stream", () => {
  ...
  function failedMidReplyChatStream(text: string) {
    ...
    return new ReadableStream<Uint8Array>({
      start(controller) {
        ...
        controller.enqueue(
          frame({ id: "chatcmpl-cut", object: "chat.completion.chunk", choices: [{ delta: { role: "assistant" } }] }),
        )
        controller.enqueue(
          frame({ id: "chatcmpl-cut", object: "chat.completion.chunk", choices: [{ delta: { content: text } }] }),
        )
        // the server gives up here — no finish_reason chunk ever arrives
        controller.enqueue(frame({ error: { message: "KV cache exhausted", type: "server_error" } }))
        controller.close()
      },
    })
  }
  ...
  it.instance(
    "releases the held tail when the provider connection dies mid-reply",
    () =>
      Effect.gen(function* () {
        const fixture = loadFixture(vivgridFixture.providerID, vivgridFixture.modelID)
        const request = waitRequest(
          "/chat/completions",
          new Response(failedMidReplyChatStream("the value is <"), {
            status: 200,
            headers: { "Content-Type": "text/event-stream" },
          }),
        )
        ...
        const sessionID = SessionID.make("session-held-tail")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        const user = {
          id: MessageID.make("msg_user-held-tail"),
          sessionID,
          role: "user",
          time: { created: Date.now() },
          agent: agent.name,
...
// origami_change: the AI SDK's warnings were being DISCARDED.
//
// `server/server.ts` and `session/prompt.ts` each set
// `globalThis.AI_SDK_LOG_WARNINGS = false` at import time - correctly, because
// the SDK's default logger is `console.warn` and the engine speaks a protocol
// on stdout. But `false` silences the whole channel, and that channel is the
// only notice the SDK gives that it dropped part of the request it was about
// to send ("Skipping reasoning parts...", the one that explains a turn which
// announces an action and then stops). `LLMAISDK.installWarningLogger` puts
// the channel back on the engine log instead of on stdout.
//
// The warning here is REAL and comes from the vendor, not from a fixture: the
// engine sends `topK: 64` for any gemini id (ProviderTransform.topK), and
// @ai-sdk/openai answers every non-null topK with
// `{ type: "unsupported", feature: "topK" }`. That makes this an end-to-end
// check of the whole path - engine option -> vendor warning -> engine log -
// rather than a check that a callback we wrote calls a logger we wrote.
describe("session.llm.stream — AI SDK warnings reach the engine log", () => {
  const vivgridFixture = { providerID: "vivgrid", modelID: "gemini-3.1-pro-preview" }

  /** Same shape test/acp/service-request-log.test.ts uses: a Logger that
   * records raw messages instead of formatting them, so the assertion is on
  ...
  it.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const sessionID = SessionID.make("session-warning-log")
        const agent = {
          name: "test",
          mode: "primary",
          options: {},
        ...
        yield* drain({
          user: {
            id: MessageID.make("msg_user-warning"),
            sessionID,
            role: "user",
            time: { created: Date.now() },
            agent: agent.name,
        ...
        yield* Effect.promise(() => request)
        // The engine really does still ask for the setting the vendor objects
        // to. Without this, a change to `topK` would leave the test asserting
        // on a warning that can no longer happen, and it would silently stop
        // covering anything. The value is absent from the request BODY on
        // purpose: dropping it is exactly what the vendor warns about.
        expect(ProviderTransform.topK(resolved)).toBeDefined()

        // The log line is written on a forked fiber, so it can land just after
        // the stream finishes. Poll rather than sleep a fixed amount.
        const found = () => capture.lines.find((line) => line.includes("provider warning"))
        for (let attempt = 0; attempt < 50 && !found(); attempt++) yield* Effect.sleep("20 millis")
