...
describe("session.retry.retryable", () => {
  ...
  // The upstream cloud upsell branches (FreeUsageLimitError / GoUsageLimitError
  // -> "subscribe to Go" actions pointing at the vendor site) were removed — this
  // fork has no cloud tier. Those bodies now take the generic retryable path.
  test("free-tier limit bodies retry generically with no upsell action", () => {
    const error = Schema.decodeUnknownSync(SessionV1.APIError.Schema)(
      new SessionV1.APIError({
    ...
    expect(SessionRetry.retryable(error, "origami")).toEqual({ message: "Free usage exceeded" })
  })
})

describe("session.retry.limit", () => {
  ...
  test("defaults to RETRY_LIMIT_DEFAULT", () => {
    expect(SessionRetry.retryLimit()).toBe(SessionRetry.RETRY_LIMIT_DEFAULT)
  })

  test("reads ORIGAMI_SESSION_RETRY_LIMIT from env, rejecting garbage", () => {
    const prev = process.env["ORIGAMI_SESSION_RETRY_LIMIT"]
    try {
      process.env["ORIGAMI_SESSION_RETRY_LIMIT"] = "3"
      expect(SessionRetry.retryLimit()).toBe(3)
      process.env["ORIGAMI_SESSION_RETRY_LIMIT"] = "0"
      expect(SessionRetry.retryLimit()).toBe(0)
      process.env["ORIGAMI_SESSION_RETRY_LIMIT"] = "not-a-number"
      expect(SessionRetry.retryLimit()).toBe(SessionRetry.RETRY_LIMIT_DEFAULT)
      process.env["ORIGAMI_SESSION_RETRY_LIMIT"] = "-1"
      expect(SessionRetry.retryLimit()).toBe(SessionRetry.RETRY_LIMIT_DEFAULT)
    } finally {
      if (prev === undefined) delete process.env["ORIGAMI_SESSION_RETRY_LIMIT"]
      else process.env["ORIGAMI_SESSION_RETRY_LIMIT"] = prev
    }
  })

  test("a persistently retryable error stops at the hard limit", () => {
    const error = retryableError()
    // Within the cap: still retries.
    expect(SessionRetry.decide(SessionRetry.RETRY_LIMIT_DEFAULT, error, retryProvider)).toEqual({
      message: "Service unavailable",
    })
    // One past the cap: gives up even though the error itself is retryable.
    expect(SessionRetry.decide(SessionRetry.RETRY_LIMIT_DEFAULT + 1, error, retryProvider)).toBeUndefined()
  })

  test("env override tightens the cap", () => {
    const prev = process.env["ORIGAMI_SESSION_RETRY_LIMIT"]
    try {
      process.env["ORIGAMI_SESSION_RETRY_LIMIT"] = "2"
      const error = retryableError()
      expect(SessionRetry.decide(2, error, retryProvider)).toBeDefined()
      expect(SessionRetry.decide(3, error, retryProvider)).toBeUndefined()
    } finally {
      if (prev === undefined) delete process.env["ORIGAMI_SESSION_RETRY_LIMIT"]
      else process.env["ORIGAMI_SESSION_RETRY_LIMIT"] = prev
    }
