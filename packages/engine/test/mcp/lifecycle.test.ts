...
import type { ServerCapabilities, Tool } from "@modelcontextprotocol/server"
...
// BEHAVIOUR CHANGE, deliberate, from the MCP v2 migration: pagination is now
// walked by the SDK, not by us, because the SEP-2549 `ttlMs`/`cacheScope` cache
// only engages on the SDK's no-cursor auto-aggregate path — hand-paginating
// opts out of caching entirely. The SDK's non-converging-server defence STOPS
// the walk on a repeated cursor (`_listAllPages`: `while (cursor !== undefined
// && !seen.has(cursor))`) where ours threw and failed the whole connect. So a
// looping server now yields a truncated list instead of a failed server. The
// two behaviours are not both available: keeping the throw means keeping
// hand-pagination means losing the cache.
it.instance("accepts empty cursors and truncates a repeated cursor instead of looping", () =>
  Effect.gen(function* () {
    ...
    looping.state.toolPages = {
      initial: { items: [{ name: "only-tool", inputSchema: { type: "object" } }], nextCursor: "repeat" },
      repeat: { items: [{ name: "repeated-tool", inputSchema: { type: "object" } }], nextCursor: "repeat" },
    }
    const mcp = yield* MCP.Service
    ...
    // Connected, and bounded: page 1 plus exactly one visit to "repeat", then
    // the walk stops. A server that never converges cannot hang the connect.
    expect(statusName(result.status, "looping-cursor")).toBe("connected")
    expect(Object.keys(yield* mcp.tools())).toEqual(["looping-cursor_only-tool", "looping-cursor_repeated-tool"])
  }),
)
...
it.instance("local stdio timeout terminates the real server process", () =>
  Effect.gen(function* () {
    ...
    const result = yield* mcp.add("hanging-stdio", {
      ...
      // Was 100ms. A connect now costs TWO spawns: the `server/discover` probe
      // sibling, then the session child once the probe's silence has been read
      // as "legacy". At 100ms the outer timeout fired mid-probe, so the session
      // child sometimes never started and the fixture published no pid at all —
      // a green-looking test measuring nothing. 2s leaves room for the probe
      // (bounded to half the budget, see mcp/index.ts `negotiation`) and the
      // fallback spawn, while the `--hang` server still guarantees the timeout.
      timeout: 2000,
    })

    expect(statusName(result.status, "hanging-stdio")).toBe("failed")
    // The fixture appends one pid per process it starts. A connect on the
    // 2026-07-28 client can start two: the `server/discover` probe sibling and
    // the session child. Both must be gone — a probe that leaks a hung process
    // on every connect would be a worse regression than the one this test was
    // written for.
    const pids = yield* pollWithTimeout(
      Effect.promise(async () => {
        const file = Bun.file(pidFile)
        if (!(await file.exists())) return undefined
        const found = (await file.text())
          .split("\n")
      ...
      // Two processes now have to start and be reaped where one used to, and
      // this file runs alongside the rest of the suite. The default 5s poll was
      // observed expiring under that load (once in six runs) while the
      // assertion itself held. The assertion is unchanged; only the patience is.
      "15 seconds",
    )
    yield* pollWithTimeout(
      Effect.sync(() =>
