...
mcpTest.instance("BrowserOpenFailed event is not published when browser launch succeeds", () =>
  Effect.gen(function* () {
    ...
    // `era` is part of a connected status since the 2026-07-28 migration. This
    // fixture is a stateful 2025 server, so the negotiation must land on legacy.
    expect(status).toEqual({ status: "connected", era: "legacy" })
