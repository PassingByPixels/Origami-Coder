...
import { PromptInput } from "@origami/schema/prompt-input"
import { Session } from "@origami/schema/session"
import { Project } from "@origami/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@origami/schema/schema"
...
import { SessionEvent } from "@origami/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this Origami process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this Origami process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
