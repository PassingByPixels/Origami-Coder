...
describe('contactThreads', () => {
  it('gives a contact who has said nothing a row anyway', () => {
    ...
    const threads = contactThreads([], [friend(CHRIS)]);
    expect(threads.map((t) => t.handle)).toEqual([CHRIS]);
  ...
  it('leads with the owner’s own label when there is one, else the name they use', () => {
    const threads = contactThreads([], [friend(CHRIS), friend(DANA, { displayName: 'Dana from the gym' })]);
    expect(threads.map((t) => t.name).sort()).toEqual(['Dana from the gym', 'chris']);
  });

  it('sorts each thread oldest-first however the mailbox arrived', () => {
    const rows = [
      row({ id: 'c', contact: CHRIS, sentAt: '2026-09-05T09:00:00.000Z' }),
      row({ id: 'a', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z' }),
      row({ id: 'b', contact: CHRIS, sentAt: '2026-09-05T08:00:00.000Z' }),
    ];
    expect(contactThreads(rows, [friend(CHRIS)])[0]!.rows.map((r) => r.id)).toEqual(['a', 'b', 'c']);
  });
  ...
  it('takes the last line from the REPLY when there is one, and its reason on a decline', () => {
    ...
    expect(contactThreads([answered], [friend(CHRIS)])[0]!.last).toBe('section 4 covers it');
    ...
    expect(contactThreads([declined], [friend(CHRIS)])[0]!.last).toBe('not over Flock');

    // MUTATION PROOF — with no reply at all it is the question, not ''.
    const asked = row({ id: 'q', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z' });
    expect(contactThreads([asked], [friend(CHRIS)])[0]!.last).toBe('q q');
  });

  it('counts waiting and unread separately: they are the two dots’ two reasons', () => {
    const rows = [
      row({ id: 'w', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z', direction: 'in', state: 'pending', unread: true }),
      row({ id: 'u', contact: CHRIS, sentAt: '2026-09-05T08:00:00.000Z', state: 'answered', unread: true }),
      row({ id: 's', contact: CHRIS, sentAt: '2026-09-05T09:00:00.000Z', state: 'sent' }),
    ];
    const thread = contactThreads(rows, [friend(CHRIS)])[0]!;
  ...
  it('orders contacts by their newest activity, and the silent ones by name', () => {
    const rows = [
      row({ id: 'old', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z' }),
    ...
    const order = contactThreads(rows, [friend(CHRIS), friend(DANA), zed, abe]).map((t) => t.name);
    expect(order).toEqual(['dana', 'chris', 'abe', 'zed']);
  });
...
describe('threadDays', () => {
  it('heads each day once, in order, and calls the current UTC day TODAY', () => {
    const rows = [
      row({ id: 'a', contact: CHRIS, sentAt: '2026-09-04T23:00:00.000Z' }),
      row({ id: 'b', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z' }),
      row({ id: 'c', contact: CHRIS, sentAt: '2026-09-05T09:00:00.000Z' }),
      row({ id: 'd', contact: CHRIS, sentAt: '2026-09-06T08:00:00.000Z' }),
    ];
  ...
  it('is driven by `now`, not by the machine clock', () => {
    const rows = [row({ id: 'a', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z' })];
  ...
  it('never merges two visits to one day across a day in between', () => {
    ...
    const rows = [
      row({ id: 'a', contact: CHRIS, sentAt: '2026-09-04T07:00:00.000Z' }),
      row({ id: 'b', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z' }),
      row({ id: 'c', contact: CHRIS, sentAt: '2026-09-04T09:00:00.000Z' }),
    ];
...
describe('defaultSelection', () => {
  const quiet = contactThreads([row({ id: 'a', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z' })], [friend(CHRIS)]);
  ...
  it('opens on All mail for an unread reply too — both are somebody’s turn', () => {
    const unread = contactThreads(
      [row({ id: 'u', contact: CHRIS, sentAt: '2026-09-05T07:00:00.000Z', state: 'answered', unread: true })],
      [friend(CHRIS)],
    );
  ...
  it('opens on the newest contact when nothing wants the owner', () => {
    expect(defaultSelection(quiet)).toBe(CHRIS);
  });
