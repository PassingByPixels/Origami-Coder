// Collab board messages, routed out of DashboardPanel.ts's switch. Ext-method calls, guards and
// payload shapes live in collabData.ts / collabBoardData.ts / collabAgentCrud.ts; this file owns
// the dispatch and the sidebar's ranked list order. Two poll sources exist: CollabPane's own fast
// poll, and a slower host-side watch (collabWatch.ts) armed from every list, so a closed tab's room
// keeps reporting.
...
/** Sidebar drag-to-reorder key for Collabs. A collab has no order field of its own, so order is
 *  persisted here and applied as a projection over the engine's own list. */
export const COLLAB_ORDER_KEY = 'origami.collabOrder';

/** The wider context the dispatcher needs from the panel — the same shape ManagerHost
 *  gives agentManager, kept fine-grained so this module never imports `vscode`. */
...
/** Every message type this dispatcher owns. Checked BEFORE DashboardPanel's own switch, mirroring
 *  AM_MESSAGE_TYPES. */
export const COLLAB_MESSAGE_TYPES = new Set([
  ...
  ...BOT_MESSAGE_TYPES,
  // Lead/objective/task board/ledger/stop — sent by the composer's slash commands and the task
  // drawer.
...
export async function handleCollabMessage(host: CollabManagerHost, m: { type?: string; [k: string]: unknown }): Promise<void> {
  switch (m.type) {
    ...
    case 'reorderCollabs': {
      // The order is saved here, not sent to the engine, then the SAME ranked projection is echoed
      // back so the sidebar settles on it.
    ...
    case 'requestCollabAgents': {
      // The `collab_agents` wire has no glyph field, and adding one would be a protocol change for
      // a presentation detail, so the GLYPH is merged in here from the same def files.
    ...
    case 'newCollab': {
      ...
      // The create form's optional objective. Absent stays absent — omitted rather than sent as an
      // empty string.
    ...
    case 'collabPost': {
      ...
      // The composer's parsed @mentions, already filtered against the active roster webview-side.
      // Strings only; undefined rather than [] when none, so the field stays off the wire.
      const named = Array.isArray(m.mentions) ? m.mentions.filter((s): s is string => typeof s === 'string') : [];
      // The composer's attachments as bare data: URLs. Strings only, undefined rather than [] when
      // none. Count/size limits are the engine's; it names which limit it hit.
    ...
    case 'collabSetConcurrency': {
      // Sent as an ORDINARY collabOpResult, not a `collabCapSet` twin: the engine can refuse this
      // one, and the op-result path already carries a refusal to the room and re-polls.
      const id = typeof m.collabId === 'string' ? m.collabId : '';
    ...
    case 'collabSetFlavor': {
      // An ORDINARY collabOpResult, like the width and unlike the cap: the engine REFUSES a council
      // whose member can still write files, and that refusal must reach the room.
      const id = typeof m.collabId === 'string' ? m.collabId : '';
    ...
    // Agent-def CRUD, bot sessions and bot memory live in botsManager.ts, routed via default:
    // below. Lead/objective/task board/ledger/stop mirror the mutations above: engine stays
    // authoritative, result payload only, no optimistic splice.
    case 'collabSetLead': {
      const id = typeof m.collabId === 'string' ? m.collabId : '';
