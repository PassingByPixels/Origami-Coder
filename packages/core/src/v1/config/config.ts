...
import { FlockConfigV1 } from "./flock"
...
/**
 * One `agentPlugins` entry. A bare string is the common, enabled case. The
 * object form exists ONLY to carry `enabled: false` — the Plugins management
 * pane (t-kgtolm round 3) writes it when a plugin is disabled, and collapses
 * it back to a plain string on re-enable, so a config a user never touched
 * through the UI stays a plain list of strings.
 */
export const AgentPluginEntry = Schema.Union([
  Schema.String,
  Schema.Struct({
    spec: Schema.String,
    enabled: Schema.optional(Schema.Boolean).annotate({
      description: "false skips this plugin on the next engine start. Defaults to true.",
    }),
  }),
]).annotate({ identifier: "AgentPluginEntry" })
export type AgentPluginEntry = Schema.Schema.Type<typeof AgentPluginEntry>
...
export const Info = Schema.Struct({
  ...
  server: Schema.optional(ConfigServerV1.Server).annotate({
    description: "Server configuration for origami serve and web commands",
  }),
  ...
  agentPlugins: Schema.optional(Schema.mutable(Schema.Array(AgentPluginEntry))).annotate({
    description:
      "Directories holding agent-plugins.org plugins to load. Each entry is a plugin root containing plugin.json (or .claude-plugin/plugin.json); ~/ is expanded and relative paths resolve from the workspace directory. A plain string is enabled; the object form carries `enabled: false` for a plugin the Plugins management pane disabled.",
  }),
  ...
  flock: Schema.optional(FlockConfigV1.Info).annotate({
    description:
      "Flock routing profiles mapping work roles to provider/model bindings. With no profile selected, routing is off and behaviour is unchanged",
  }),
    ...
    Schema.Struct({
      ...
      tool_search: Schema.optional(
        Schema.Struct({
          enabled: Schema.optional(Schema.Boolean).annotate({
            description:
              "Hide deferred tools behind the tool_search catalog instead of sending every schema each turn (default: true)",
          }),
          mcp: Schema.optional(Schema.Boolean).annotate({
            description: "Defer MCP server tools by default, so they cost one catalog line until searched (default: true)",
          }),
          defer: Schema.optional(Schema.mutable(Schema.Array(Schema.String))).annotate({
            description:
              "Extra tool ids to defer. '*' is the only wildcard, so one server is 'server_*' and one tool is its exact id",
          }),
          always: Schema.optional(Schema.mutable(Schema.Array(Schema.String))).annotate({
            description: "Tool ids that are never deferred, whatever the defaults say. Same '*' wildcard as 'defer'",
          }),
        }),
      ).annotate({
        description: "Deferred tool catalog: which tools cost a catalog line until tool_search loads their schema",
      }),
    }),
  ),
