...
export class ConfigService extends Context.Service<ConfigService, Config>()("@origami/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.ORIGAMI_WEBSEARCH_PROVIDER === "exa" || process.env.ORIGAMI_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.ORIGAMI_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("ORIGAMI_EXPERIMENTAL") || truthy("ORIGAMI_ENABLE_EXA") || truthy("ORIGAMI_EXPERIMENTAL_EXA"),
    enableParallel: truthy("ORIGAMI_ENABLE_PARALLEL") || truthy("ORIGAMI_EXPERIMENTAL_PARALLEL"),
