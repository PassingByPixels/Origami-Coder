...
/**
 * Wall-clock ceiling on ONE job. Nothing else bounded a background job: the
 * registry started work, kept a handle to it, and then waited forever - a job
 * whose process wedged stayed `running` for the life of the instance, counted
 * as outstanding by every caller that asks, and could only be ended by a user
 * who happened to notice. Thirty minutes is long enough for the work this
 * registry is actually given (a sub-agent turn, a build, a test run) and short
 * enough that a wedged job is a nuisance rather than a permanent one.
 */
...
export const make = Effect.gen(function* () {
    ...
    const result = yield* SynchronizedRef.modify(state.jobs, (jobs): readonly [FinishResult, Map<string, Active>] => {
      ...
      const next = {
        ...
        info: {
          ...
          completed_at,
          error: `background task exceeded its maximum duration of ${ms} ms and was cancelled`,
          metadata: { ...job.info.metadata, expired: true },
        },
      }
      return [{ info: snapshot(next), done: job.done, scope: job.scope }, new Map(jobs).set(id, next)]
    })
   ...
   * finishes normally takes the watchdog fiber down with it. `extend` on a
   * running job deliberately does NOT re-arm it: the ceiling is on the job's
   * total life, and a chain of extensions is exactly the runaway shape it
   * exists to bound.
   */
  const watchdog = (id: string, token: object, done: Deferred.Deferred<Info>, ms: number) =>
    Deferred.await(done).pipe(
      ...
      Effect.flatMap((settled) => (settled._tag === "None" ? expire(id, token, ms) : Effect.void)),
      Effect.asVoid,
    ...
    return yield* Effect.uninterruptibleMask((restore) =>
      Effect.gen(function* () {
        ...
        if ("scope" in result) {
          ...
          if (Number.isFinite(ceiling) && ceiling > 0) yield* watchdog(id, result.token, done, ceiling)
        }
        return result.info
      }),
    )
