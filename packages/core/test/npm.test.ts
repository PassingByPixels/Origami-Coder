...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@origami/acme")).toBe("@origami/acme")
    expect(Npm.sanitize("@origami/acme@1.0.0")).toBe("@origami/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/origami/acme.git"
    const expected = win ? "acme@git+https_//github.com/origami/acme.git" : spec
