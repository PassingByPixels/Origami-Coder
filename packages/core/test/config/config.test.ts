...
import { Config } from "@origami/core/config"
import { ConfigProvider } from "@origami/core/config/provider"
import { AppNodeBuilder } from "@origami/core/effect/app-node-builder"
import { LayerNode } from "@origami/core/effect/layer-node"
import { ConfigMigrateV1 } from "@origami/core/v1/config/migrate"
import { ConfigV1 } from "@origami/core/v1/config/config"
import { FSUtil } from "@origami/core/fs-util"
import { Global } from "@origami/core/global"
import { Location } from "@origami/core/location"
import { Policy } from "@origami/core/policy"
...
describe("Config", () => {
  ...
  it.live("loads origami JSON and JSONC files from lowest to highest priority", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(
                path.join(tmp.path, "origami.json"),
                JSON.stringify({ $schema: "base", providers: { base: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "origami.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.path).toBe(path.join(tmp.path, "origami.json"))
            expect(documents[1]?.info.providers?.last).toBeInstanceOf(ConfigProvider.Info)

            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "origami.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              fs.writeFile(path.join(tmp.path, "origami.json"), JSON.stringify({ $schema: "base" })),
              fs.writeFile(path.join(tmp.path, "origami.jsonc"), "{ invalid"),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
  ...
  it.live("loads global, ancestor, and .origami configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".origami"), { recursive: true })
            await fs.mkdir(path.join(directory, ".origami"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "origami.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "origami.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "origami.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "origami.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "origami.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".origami", "origami.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".origami")),
              AbsolutePath.make(path.join(directory, ".origami")),
            ])
