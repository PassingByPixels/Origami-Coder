...
import { ModelsDevPlugin } from "@origami/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.ORIGAMI_MODELS_PATH,
          disabled: Flag.ORIGAMI_DISABLE_MODELS_FETCH,
        }
        Flag.ORIGAMI_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.ORIGAMI_MODELS_PATH = previous.path
          Flag.ORIGAMI_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
