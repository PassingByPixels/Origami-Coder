...
// No default: the only candidate default was the UPSTREAM repo, and a changelog
// silently generated from somebody else's project is worse than a loud stop.
const repo = process.env.GH_REPO
if (!repo) throw new Error("GH_REPO is not set (owner/repo of the fork's own GitHub repository)")
const bot = ["actions-user", "github-actions[bot]", "origami", "origami-agent[bot]"]
const team = [
...
async function commits(from: string, to: string) {
  ...
  const log =
    await $`git log ${base}..${head} --format=%H -- packages/engine packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/engine/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/engine/")) areas.add("core")
